...
Defined at module_drivers/drivers/dma/ingenic/Kconfig:1 
...
Selected by [y]: - SND_ASOC_PDMA [=y] && SND_ASOC_INGENIC [=y] && SND_ASOC_INGENIC_AS_V1 [=y] 
```
...
kernel4.4.94和kernel5.10在使用的上的差异主要是kernel5.10去除了部分api和kernel4.4.94申请dma channel的api接口不一样，用户可参考12.8.2.2。
...
[ 148.309825] dmatest: dma0chan31-copy: summary 1 tests, 0 failures 539 iops 5399 KB/s (0)
```

### SSI驱动程序测试

PDMA驱动遵循内核标准的dmaengine驱动框架，可作为provider为其他驱动程序提供dma传输通道，这里以SSI驱动程序为例，介绍如何配置。
...
dmas表示使用PDMA提供的对应传输通道

dma-names为一个标志，用于在SSI驱动程序中申请dma传输通道
