#ifndef LINEAR_FIXPOINT_MSA_H
#define LINEAR_FIXPOINT_MSA_H
...
namespace tnn {
class Linear_fixpoint_msa :public layer {
 public:

  float cvt16fto32f(uint16_t w) const;
  uint16_t cvt32fto16f(float x) const;
  Linear_fixpoint_msa(std::size_t hidden_size,std::size_t nOut) :szHidden(hidden_size),szOut(nOut)
  {
    m_weight = tensor_uint16{nOut,2 * hidden_size};
    m_weight_8 = tensor_int8{nOut,2 * hidden_size};
    m_bias = tensor_float{nOut};
  }
  virtual ~Linear_fixpoint_msa(){
  }
 ...
 private:
  ...
  tensor_float m_bias;
  tensor_float factor_m_weight;
  tensor_uint16 m_weight;
  tensor_int8 m_weight_8;
  std::size_t szOut;
  std::size_t szHidden;
};
}

#endif /* LINEAR_FIXPOINT_MSA_H */
