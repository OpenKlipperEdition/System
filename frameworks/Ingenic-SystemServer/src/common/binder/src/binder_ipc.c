...
int ti_write_outbuf(tIpcThreadInfo *ti, void *buf, size_t sz) {
  if (x_unlikely(!ti) || x_unlikely(!buf))
    return -1;
  if (ti->out_buf.size - ti->out_buf.consumed < sz)
    return -1;
...
int talk_with_driver(tIpcThreadInfo *ti, int read_flag) {
  ...
  if (!w_buf && !r_buf)
    return 0; /*we should still handle the in_data in this case.*/

  if (w_buf)
    binder_buf_init(&tmp_w_buf, w_buf->ptr, w_buf->consumed, 0);

  if ((ret = binder_write_read(ti->bs, w_buf ? &tmp_w_buf : NULL, r_buf)) >=
      0) {
    if (w_buf) {
      ...
      case BR_REPLY:
        if (reply) {
          ...
          if (txn->flags & TF_STATUS_CODE) {
            /*it's a status code, means functions call return a error code, we
             * parse it and free the buffer*/
            err = *((int32_t *)(txn->data.ptr.buffer));
            binder_cmd_freebuf(ti, (void *)txn->data.ptr.buffer);
          }
        } else {
          /*this case should not be come in if every one write the correct
           * function call. we add it to avoid binder buufer not freed.
           */
          struct binder_transaction_data *txn =
              (struct binder_transaction_data *)data;
 ...
 */
static int _binder_cmd_call(tIpcThreadInfo *ti, tBinderIo *bio,
                            tBinderIo *reply, uint32_t target, uint32_t code,
                            int flag) {
  int ret = 0;
  ...
  if (flag) {
    ...
    /*sync call will receive  BR_TRANSACTION_COMPLETE from kernel and BR_REPLY
     * from target*/
...
int binder_cmd_sync_call(tIpcThreadInfo *ti, tBinderIo *bio, tBinderIo *msg,
                         uint32_t target, uint32_t code) {
  return _binder_cmd_call(ti, bio, msg, target, code, 0);
}

int binder_cmd_async_call(tIpcThreadInfo *ti, tBinderIo *bio, tBinderIo *msg,
                          uint32_t target, uint32_t code) {
...
int binder_cmd_unlink_to_death(tIpcThreadInfo *ti, uint32_t target, void *death) {
  struct {
    uint32_t cmd;
    struct binder_handle_cookie payload;
  } __attribute__((packed)) death_data;

  death_data.cmd = BC_CLEAR_DEATH_NOTIFICATION;
  death_data.payload.handle = target;
  death_data.payload.cookie = (binder_uintptr_t)death;
...
int binder_execute_cmds(tIpcThreadInfo *ti, uint32_t cmd, void *data) {
  ...
  case BR_TRANSACTION: {
    ...
    if (tr->target.ptr) {
      ...
      binder_io_init(&reply, binder_buf, sizeof(binder_buf),
                     DEFAULT_OFFSET_LIST_SIZE);
      if (b_srv && b_srv->transact_cb)
        status_flag = b_srv->transact_cb(tr->code, &bio, &reply, tr->flags);
    }
...
//    b_srv = (tBinderService *)__ptr;
    struct binder_death *death = (struct binder_death *) __ptr;
...
int binder_add_service(const char *name, tBinderService *b_srv) {
  ...
  binder_io_init(&bio, binder_buf, sizeof(binder_buf),
                 DEFAULT_OFFSET_LIST_SIZE);
...
uint32_t binder_check_service(const char *service_name) { return 0; }
...
size_t binder_del_service(const char *service_name) {
  tIpcThreadInfo *ti = binder_get_thread_info();
  char binder_buf[DEFAULT_BINDER_IOBUF_SIZE] = {0};
  tBinderIo bio, reply;
  uint32_t cmd = BC_TRANSACTION;
  ...
  if (BINDER_STATUS_OK == _binder_cmd_wait_rsp(ti, &reply)) {
   	uint32_t ret = binder_io_get_uint32(&reply);
