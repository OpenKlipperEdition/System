![](assets/01-hippo-uboot开发手册_v2.0.0.png)
**君正®**
...
Ingenic芯片平台u-boot引导流程如下图所示：

![](assets/01-hippo-uboot开发手册_v2.0.1.png)从上图可看到，分为两种引导方式：
...
以Hippo_v12开发板为例，决定u-boot配置的文件主要有：

1. **u-boot/boards.cfg**
2. **u-boot/include/configs/hippo.h**
...
2. ARCH、CPU、Board name、Vendor、SoC：主要用于Makefile编译时选择代码目录，比如u-boot/include/configs目录下有许多配置文件，选择hippo.h便是通过Board name确定的；

**3. Options：每一种配置之间的主要区别，u-boot会为附加选项中的每个字段前面加上“CONFIG_”前缀并导出为全局可用的宏定义，这些宏定义会进一步决定其他的配置，目前主要是hippo.h中的配置。**

### 默认配置

根据不同的启动介质和硬件链接情况，会有通的配置，以Hippo为例共支持六种编译配置(其它开发板类似)，如下表所示:



|                               |                                                                                                                                                                                                                                                                                           |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| hippo_uImage_msc1           | **该配置支持从emmc启动linux内核镜像uImage**CONFIG_SPL_JZMMC_SUPPORT：spl中的mmc相关配置CONFIG_ENV_IS_IN_MMC：环境变量存储相关配置CONFIG_GPT_CREATOR：分区相关配置CONFIG_JZ_MMC_MSC1：msc控制器相关配置CONFIG_SPL_PARAMS_FIXER: 改变CPM频率                                 |
| hippo_uImage_sfc_nand      | **该配置支持从nand flash启动linux内核镜像uImage**CONFIG_SPL_SFC_NAND：sfc nand flash相关配置CONFIG_MTD_SFCNAND：sfc nand flash驱动编译配置CONFIG_SPL_PARAMS_FIXER: 改变CPM频率                                                                                                    |
| hippo_uImage_sfc_nor       | **该配置支持从nor flash启动linux内核镜像uImage**CONFIG_SPL_SFC_NOR：sfc nor flash相关配置CONFIG_ENV_IS_IN_SFC：环境变量存储相关配置CONFIG_MTD_SFCNOR：sfc nor flash驱动编译相关配置CONFIG_SPL_PARAMS_FIXER: 改变CPM频率                                                       |
| hippo_xImage_sfc_nand_ota | **该配置支持从nand flash启动linux内核镜像xImage**CONFIG_SPL_SFC_NAND：sfc nand flash相关配置CONFIG_MTD_SFCNAND：sfc nand flash驱动编译相关配置CONFIG_SPL_OS_BOOT：spl直接启动linux内核镜像相关配置CONFIG_OTA_VERSION30：ota升级相关配置CONFIG_SPL_PARAMS_FIXER: 改变CPM频率  |
| hippo_xImage_sfc_nand      | **该配置支持从nand flash启动linux内核镜像xImage**CONFIG_SPL_SFC_NAND：sfc nand flash相关配置CONFIG_MTD_SFCNAND：sfc nand flash驱动编译相关配置CONFIG_SPL_OS_BOOT：spl直接启动linux内核镜像相关配置CONFIG_SPL_PARAMS_FIXER: 改变CPM频率                                         |
| hippo_xImage_sfc_nor       | **该配置支持从nor flash启动linux内核镜像xImage**CONFIG_SPL_SFC_NOR：sfc nor flash相关配置CONFIG_MTD_SFCNOR：sfc nor flash驱动编译相关配置CONFIG_SPL_OS_BOOT：spl直接启动linux内核镜像相关配置CONFIG_ENV_IS_IN_SFC：环境变量存储相关配置CONFIG_SPL_PARAMS_FIXER:改变CPM频率 |

### 自定义配置

可根据实际需要，在每种板级配置中增删相应的字段或者新添加一个新的板级配置。

下面以添加一个新的板级配置hippo_uImage_msc1为例进行介绍：

1. 在boards.cfg文件中新起一行，按照该文件的格式要求，先确定第一个配置项为hippo_uImage_msc1，第二到第六个配置项可参考现存的板级配置，一般都是比较固定的；
2. 主要是附加选项的配置，如果对hippo平台的u-boot整体配置不是很了解的话，可以先参考现存配置，比如这里可以参考hippo_uImage_msc0，将JZ_MMC_MSC1改为JZ_MMC_MSC0，这样会选择MSC0相关的配置，其他的保持一致即可；
3. 如果想更加细致的修改附加选项，建议先整体阅读一遍该文档，尤其是第二章关于配置的介绍，在了解了hippo平台的u-boot整体配置后，再进行修改会更加游刃有余。

## Uboot板级配置文件说明

以Hippo开发板为例，hippo.h为u-boot中针对Hippo平台核心的配置文件，该配置文件主要包含以下内容：
...
不同的芯片内置的DDR类型是不同的，需要根据实际情况选择DDR类型：
...
1. DDR3配置，如果定义**CONFIG_DDR_TYPE_DDR3**，则DDR3相关的配置会被打开：
```c
...
#endif
```
1. LPDDR3配置，如果定义**CONFIG_DDR_TYPE_LPDDR3**，则LPDDR3相关配置会被打开：
```c
...
#endif
```
1. LPDDR2配置，如果定义**CONFIG_DDR_TYPE_LPDDR2**，则LPDDR2相关配置会被打开：
```c
...
```
1. sfc nand flash配置，如果定义**CONFIG_SPL_SFC_NAND**，则nand相关的配置会被打开：
```c
/* sfc nand config */
...
#endif
```
1. sfc nor flash配置，如果定义**CONFIG_SPL_SFC_NOR**，则nor相关的配置会被打开：
```c
/* sfc nor config */
...
1. 使用nand flash启动，在boards.cfg文件的**hippo_uImage_sfc_nand**配置和**hippo_xImage_sfc_nand**配置中导出了**CONFIG_SPL_SFC_NAND。**
...
#endif
```
1. MSC1配置，如果定义**CONFIG_JZ_MMC_MSC1**，则MSC1相关的配置会被打开：
```c
...
#endif
```
1. MSC2配置，如果定义**CONFIG_JZ_MMC_MSC2**，则MSC2相关的配置会被打开：
```c
...
使用emmc启动，在boards.cfg文件的hippo**_uImage_msc1**配置中导出了**CONFIG_JZ_MMC_MSC1**，即当前默认使用MSC1配置。
...
在u-boot/board/ingenic/hippo/partitions.tab中存放有具体的分区信息：
```c
property:
...
```
1. 网口工作模式选择，需要按照实际连接的网络phy芯片进行选择
```c
...
2.根据实际速度需要，工作模式可选择RGMII或者RMII，其中RGMII支持10/100/1000兆的总线接口速度，RMII支持10兆和100兆的总线接口速度，不同的芯片支持的工作模式有所不同。
...
以hippo开发板为例 ，配置文件hippo.h中，需要将下面的宏定义打开
```c
#define CONFIG_LCD

#define CONFIG_JZ_LCD_V14
```
LCD-Panel相关的配置，以LCD-Fw050为例，需要将相关引脚的定义和硬件连接修改一致
```c
...
以Hippo_v10开发板为例，此开发板以SPI Nand Flash存储介质。 

1.在工程顶层目录下执行如下命令，以hippo平台所需的环境变量：
```c
...
1.hippo.v10_nand_4.4.94-eng

4. hippo.v10_nand_5.10-eng
...
$ make uboot -j12
```
5.等待编译完成，在工程/out/product/hippo/image目录下便会生成uboot镜像文件，该镜像即可直接进行烧录。

## u-boot源码目录编译

1. 在工程顶层目录下执行如下命令，导入编译hippo平台所需的环境变量：
```c
$ source build/envsetup.sh
```
1. 执行lunch命令选择需要编译镜像的类型， 这里只是为了导入交叉编译工具环境，如果只编译 u-boot可以随意选择：
```c
$ lunch
```
1. 之后进入u-boot目录，执行make distclean清除上次编译产生的中间产物：
```c
$ make distclean
```
执行make xxxxxx -jN命令进行编译，xxxxxx为具体的板级配置名称，需要根据编译的启动 镜像来决定，详细介绍请参考[2.1 boards.cfg文件说明](#_boards.cfg文件说明)。以hippo平台为例，具体如下表所示：



|                          |                                               |
| ------------------------ | --------------------------------------------- |
| hippo_uImage_msc1      | 该配置支持从emmc启动linux内核镜像uImage       |
| hippo_uImage_sfc_nand | 该配置支持从nand flash启动linux内核镜像uImage |
| hippo_uImage_sfc_nor  | 该配置支持从nor flash启动linux内核镜像uImage  |
| hipoo_xImage_sfc_nand | 该配置支持从nand flash启动linux内核镜像xImage |

比如，编译从nand flash启动uImage的启动镜像，执行如下命令：

$ make hippo_uImage_sfc_nand -jN

其中-jN为使用多线程编译，可以加快编译速度，具体的N值需要根据使用的PC决定。

1. 编译好之后，会在u-boot工程顶层目录下生成u-boot-with-spl.bin，该文件便是最终生成的镜像文件，可以直接进行烧录。
2. 特别注意的是，如果编译的是MSC启动的uboot，生成用于烧录的文件是u-boot-with-spl-mbr-gpt.bin。
...
![](assets/01-hippo-uboot开发手册_v2.0.2.png)![](assets/01-hippo-uboot开发手册_v2.0.3.png)对于每一个命令，还可以执行“help 命令名称”获取该命令具体的使用说明，如下图所示：

![](assets/01-hippo-uboot开发手册_v2.0.4.png)**下面介绍一些常用的命令：**
...
具体的，我们可以通过在hippo.h中将启动参数设置给宏定义CONFIG_BOOTARGS。

在hippo.h中，根据加载内核时挂载的文件系统的不同均有对应的启动参数，它们有一些共有的内容，在hippo.h中定义如下：
```c
...
网络文件系统一般用于开发时调试使用，当前在hippo.h中的参考启动参数如下：
```c
...
该启动参数对应**hippo_uImage_msc2**配置**，**当前在hippo.h中的启动参数如下：
```c
#define CONFIG_BOOTARGS 
...
该启动参数对应**hippo_uImage_sfc_nand**配置**，**当前在hippo.h中的启动参数如下：
```c
#define CONFIG_BOOTARGS 
...
1. uboot只支持一个gmac控制器工作，需要在hippo.h中配置选择的控制器和对应控制器的接口模式：
```c
/* Select GMAC Controller */
...
```c
hippo# set ipaddr 192.168.4.145

hippo# set serverip 192.168.4.146
```
b.将需要的uImage放入PC服务器文件夹下，执行以下命令将uImage下载到内存 0x80800000位置处：
```c
hippo# tftp 0x80800000 uImage
```
 c.启动uImage：
```c
hippo# bootm 0x80800000
```
...
```c
hippo# loady 0x80800000 115200
## Ready for binary (ymodem) download to 0x80800000 at 115200 bps...
C
```
1. 先按住ctrl键不放，接着按住a键，再同时放开两个按键，最后按一下s键，可以看到如下图所示画面：

![](assets/01-hippo-uboot开发手册_v2.0.5.png)这里按↓键选择ymodem协议，然后回车；

1. 然后会出现如下图所示画面，这里可以选择需要传输的文件，↑↓键进行移动，双敲空格键进入目录，单敲空格键选择文件，回车键确认选择。

我们选择需要传输的kernel镜像：

![](assets/01-hippo-uboot开发手册_v2.0.6.png)1. 选择完毕并回车确认后，会出现如下进行文件的传输的画面：

![](assets/01-hippo-uboot开发手册_v2.0.7.png)1. 等待传输完毕，便可以通过bootm命令启动写到内存0x80800000位置处的kernel镜像:
```c
$ bootm 0x80800000
```
# Ingenic tools介绍
...
├── nand_device
```
1. 保存nand flash和nor flash的分区参数，区别于由烧录工具写入分区参数，最终会由kernel中的sfc驱动读取并生成对应的mtd分区，如有需要可自行更改
```c
├── sfc_builtin_params
│   ├── nand_device.c
│   ├── nand_device.h
...
│   └── nor_device.h
```
1. 用于写入nand flash和nor flash的分区参数到u-boot镜像中的指定位置
```c
...
├── sfc_timing_params.c
```
1. 用于sfc boot时生成spl前面的头部数据，为bootom加载spl作校验用
```c
├── spi_checksum.c
```
1. x2000快速启动时使用，用于生成spl env参数
```c
