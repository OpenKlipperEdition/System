...
SSI是一个全双工同步串行接口，可以连接到多种外部模拟-数字（A/D）转换器、音频和电信编解码器以及其他使用串行传输数据的协议。X26xx系列Soc支持SSI摩托罗拉的串行外设接口（SPI）协议。
...
设备树默认关闭SPI控制器设备，因为几组SPI功能引脚，被其他模块占用。 
...
&spi0 {
	...
	ingenic,spi-mux-select = "epll"; /* 选择mux clock parent: sclka; epll; mpll */
...
| ingenic,spi-src-clk | 设备时钟源的选择，1表示内部时钟，不支持外部时钟 |
| ingenic,spi-mux-select = "epll" | /* 选择mux clock parent: sclka; epll; mpll */ |
| num-cs | 控制器有几根片选pin |
...
注意：根据“Documentation/devicetree/bindings/spi/spi-gpio.yaml”文件中的描 述，kernel5.10中已经废弃原有的属性“gpio-sck”、“gpio-miso”和“gpio-mosi”， 但实际kernel5.10内核依然保留了对原有属性的解析，建议采用新的devicetree属性。
...
2. 执行**spidev_test -D /dev/spidev0.0**
