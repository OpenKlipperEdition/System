...
x2000使用的是AP6256芯片，AP6256是基于BCM4345C5方案的集成wifi和bluetooth的功能模块。
...
| **PCM_SYNC** | I/O | PCMsyncsignal |

## 驱动源码位置

驱动源码所在位置：

***kernel/module_drivers/drivers/misc/***
...
在板级设备树halley5_v30.dts中，进行如下默认配置：
...
Prompt: Bluetooth power control driver for BCM-4345C5 module 
 ...
 Defined at module_drivers/drivers/misc/Kconfig 

 Depends on: RFKILL [=y] && (MMC_SDHCI_INGENIC [=y] || INGENIC_MMC_MMC0 [=n] || \ 
...
2. 执行bsa_server,启动服务
```bash
# bsa_server -r 15 -p /firmware/BCM4345C5_003.006.006.0058.0135.hcd -u /var/run/ -d /dev/ttyS0
```
...
![9b412fd4a055c98adcda273be484dee](assets/BT蓝牙.0.jpeg)

检查低功耗节点不存在

![67a78d0815089e19c7b9c731312fd7e](assets/BT蓝牙.1.jpeg)
