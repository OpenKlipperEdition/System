...
/*************************************************************************//*!
   ...
   retrieve the error code
*****************************************************************************/
bool AL_Encoder_SetBitRate(AL_HEncoder hEnc, int iBitRate, int iMaxBitRate);
...
#if AL_ENABLE_TRACES
/*************************************************************************//*!
   \brief Specifies the location where output trace files will be generated.
   \param[in] hEnc Handle to an encoder object
   \param[in] sPath Pointer to a null-terminated string that specifies the name
   of an existing folder. This pointer must remain valid until
   next call to SetTraceFolder or DestroyEncoder.
*****************************************************************************/
void AL_Encoder_SetTraceFolder(AL_HEncoder hEnc, const char* sPath, const char* sPrefix);

/*************************************************************************//*!
   \brief Specifies how the encoder should dump the output trace files for next encoded slice(s).
   \param[in] hEnc Handle to an encoder object
   \param[in] iMode Specifies output trace mode :
   0 = Do not generate trace files
   1 = Generate trace files for all frame
   2 = Generate trace files only when errors occur
   3 = Generate trace for a single frame identified by iParam
   \param[in] iParam When iMode = 3 specified the frame number in encoding order
   \param[in] shouldPrintFrameDelimiter prints start_of_frame / end_of_frame each time a frame starts / ends
*****************************************************************************/
void AL_Encoder_SetTraceMode(AL_HEncoder hEnc, int iMode, int iParam, bool shouldPrintFrameDelimiter);

#include "lib_common_enc/EncHook.h"
typedef struct
{
  void* pUserParam;
  void (* RecordStart)(void*, AL_EEncHook, int);
  void (* RecordEnd)(void*, AL_EEncHook, int);
}AL_EncTraceHook;

void AL_Encoder_SetTraceHooks(AL_HEncoder hEnc, AL_EncTraceHook* pHooks);

/****************************************************************************/

bool AL_Encoder_GetFrameRate(AL_HEncoder hEnc, uint16_t *uFrameRate, uint16_t *uClkRatio);
bool AL_Encoder_GetRcParam(AL_HEncoder hEnc, AL_TRCParam *pRcParam);
bool AL_Encoder_GetGopParam(AL_HEncoder hEnc, AL_TGopParam *pGopParam);

bool AL_Encoder_SetRcParam(AL_HEncoder hEnc, AL_TRCParam *pRcParam);
bool AL_Encoder_SetGopParam(AL_HEncoder hEnc, AL_TGopParam *pGopParam);
#endif
