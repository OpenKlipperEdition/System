...
设备树默认编译会产生otg 和otg_phy设备，在halley5_v30.dts中配置如下：
```c
&otg {
...
| --- | --- |
| **属性名称** | **说明** |
| ingenic,id-dete-gpio| 配置usb用于探测id管脚变化的GPIO  |
| ingenic,vbus-dete-gpio| 配置usb用于插拔检测的GPIO  |
| Ingenic,drvvbus-gpio| 配置usb用于控制vbus对外供电的GPIO  |
...
或者可在halley5_v30.dts中配置USB控制器模式：
```c
&otg {
...
X2000 USB IP遵循USB2.0规范，支持多种测试模式，包括Test_J模式、Test_K模式、Test_SE0_NAK 模式、Test_Packet模式和Test_Force_Enable模式。

1、作为device进行眼图测试，需要利用电脑端测试软件工具 HSETT，使其进入test_Packet测试 模式。

2、作为host进行眼图测试，需要配置寄存器HPRT[16,13]位，使其进入test_Packet测试模式。

**注：** 进入测试模式方法参考 zh-cn/linux-app-notes/USB信号质量测试.md

x2000 usb Phy的IP厂商预留6bit用于调整眼图：

3bit 调整幅度:通过调整参考电压来调整45Ω的HS终端电阻。有助于补偿封装和电路板级寄生效应。降低补偿电压,降低 ODT 阻抗。

3bit 调整斜率:通过调整参考电压来调整HS电流值。

调整方法：

usb phy寄存器映射到总线基地址：0x10078000 ; 调节眼图的寄存器地址：0x10078060
