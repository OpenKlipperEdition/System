...
#define TAG "h264dec_v1"

#define V4L2_PIX_FMT_JZ420B  v4l2_fourcc('J', 'Z', '1', '2')
...
static int _h264_dec_srcfmt_set(void *chn_ctx)
{
	assert(chn_ctx);
	h264d_chn_ctx_t *ctx = (h264d_chn_ctx_t *)chn_ctx;
	...
	width = ctx->h264d_param.src_width;
	height = ctx->h264d_param.src_height;
    ...
    ret = ioctl(ctx->fd, VIDIOC_G_FMT, &format);
    if (ret) {
            IMPP_LOGE(TAG, "encoder srcfmt get failed.");
            return -IHAL_RERR;
    }

    if (ctx->srcfmt_des->v4l2_mplane) {
            ctx->h264d_param.src_width = format.fmt.pix_mp.width;
            ctx->h264d_param.src_height = format.fmt.pix_mp.height;
            ctx->image_width = format.fmt.pix_mp.width;
            ctx->image_height = format.fmt.pix_mp.height;
    } else {
            ctx->h264d_param.src_width = format.fmt.pix.width;
            ctx->h264d_param.src_height = format.fmt.pix.height;
            ctx->image_width =format.fmt.pix.width;
            ctx->image_height = format.fmt.pix.height;
    }

	return IHAL_ROK;
}

static int _h264_dec_dstfmt_set(void *chn_ctx)
{
	assert(chn_ctx);
	h264d_chn_ctx_t *ctx = (h264d_chn_ctx_t *)chn_ctx;
...
static int _init_mmap(h264d_chn_ctx_t *ctx, int type, void *bufferinfo, int num)
{
	int ret = 0;
...
static int _init_dmabuf(h264d_chn_ctx_t *ctx, int type, int num)
{
	struct v4l2_requestbuffers reqbuf;
	int ret = 0;
...
static int _init_userptr(h264d_chn_ctx_t *ctx, int type, int num)
{
	struct v4l2_requestbuffers reqbuf;
	int ret = 0;
	assert(ctx);
...
static int _h264_dec_init_exit_buffers(video_data_buffer_t *pbuf, int width, int height, int planes, unsigned int *startaddr, unsigned int totalsize)
{
	if (planes > 3) {
		return -IHAL_RERR;
	}
...
static int _h264_dec_qDst(void* chn_ctx, int idx)
{
	assert(chn_ctx);
	h264d_chn_ctx_t *ctx = (h264d_chn_ctx_t *)chn_ctx;
...
static int h264_dec_init(void)
{
	return IHAL_ROK;
}

static int h264_dec_deinit(void)
{
	return IHAL_ROK;
}

static void* h264_dec_chn_ctx_create(void)
{
	h264d_chn_ctx_t *chn_ctx = NULL;

	chn_ctx = (h264d_chn_ctx_t *)calloc(1, sizeof(h264d_chn_ctx_t));
	if (!chn_ctx)
		return IHAL_RNULL;

	chn_ctx->fd = open("/dev/video2", O_RDWR);
	if (!chn_ctx->fd) {
		free(chn_ctx);
		chn_ctx = NULL;
		IMPP_LOGE(TAG, "open h264dec decoder failed.");
		return IHAL_RNULL;
	}
...
static int h264_dec_chn_ctx_destroy(void* chn_ctx)
{
	assert(chn_ctx);
...
static int h264_dec_param_set(void *chn_ctx, IHal_CodecParam *param)
{
	assert(chn_ctx);
	assert(param);
	...
	memcpy(&ctx->h264d_param, &param->codecparam.h264d_param, sizeof(IHal_H264D_Param));

	ctx->srcfmt_des = &formats[2];

	switch (param->codecparam.h264d_param.dst_fmt) {
		case IMPP_PIX_FMT_NV12:
			ctx->dstfmt_des = &formats[0];
			break;
		case IMPP_PIX_FMT_NV21:
			ctx->dstfmt_des = &formats[1];
			break;
		case IMPP_PIX_FMT_JZ420B:
			ctx->dstfmt_des = &formats[3];
			break;
		default:
			IMPP_LOGW(TAG, "dst_fmt type(%d) error!", param->codecparam.h264d_param.dst_fmt);
			return -IHAL_RERR;
	}

	ret = _h264_dec_srcfmt_set(ctx);
	if (ret) {
		IMPP_LOGE(TAG, "src fmt set error.");
		return -IHAL_RERR;
	}

	ret = _h264_dec_dstfmt_set(ctx);
	if (ret) {
		IMPP_LOGE(TAG, "dst fmt set error.");
		return -IHAL_RERR;
	}

	memcpy(&param->codecparam.h264d_param, &ctx->h264d_param, sizeof(IHal_H264D_Param));

	return IHAL_ROK;
}

static int h264_dec_param_get(void *chn_ctx, IHal_CodecParam *param)
{
	return IHAL_ROK;
}

static int h264_dec_chn_start(void* chn_ctx)
{
	assert(chn_ctx);
	h264d_chn_ctx_t *ctx = (h264d_chn_ctx_t *)chn_ctx;
	int ret = 0;
	...
	for (int i = 0; i < ctx->dst_buf_num; i++) {
		ret = _h264_dec_qDst(chn_ctx, i);
		if (ret) {
			IMPP_LOGE(TAG, "h264dec start dst buffer prepare failed.");
			return -IHAL_RERR;
		}
	}
...
static int h264_dec_chn_stop(void* chn_ctx)
{
	assert(chn_ctx);
	h264d_chn_ctx_t *ctx = (h264d_chn_ctx_t *)chn_ctx;
	enum v4l2_buf_type type;
...
static int h264_dec_srcbuffer_create(void* chn_ctx, IMPP_BUFFER_TYPE buftype, int num)
{
	assert(chn_ctx);
	h264d_chn_ctx_t *ctx = (h264d_chn_ctx_t *)chn_ctx;
	int ret = 0;
...
static int h264_dec_srcbuffer_free(void* chn_ctx)
{
	assert(chn_ctx);
...
static int h264_dec_dstbuffer_create(void* chn_ctx, IMPP_BUFFER_TYPE buftype, int num)
{
	assert(chn_ctx);
...
static int h264_dec_dstbuffer_free(void* chn_ctx)
{
	assert(chn_ctx);
...
static int h264_dec_srcbuffer_get(void* chn_ctx,IMPP_BufferInfo_t *buffer,int index)
{
...
static int h264_dec_srcbuffer_set(void* chn_ctx,IMPP_BufferInfo_t *buffer,int index)
{
	/* only support userptr */
	assert(chn_ctx);
	h264d_chn_ctx_t *ctx = (h264d_chn_ctx_t *)chn_ctx;
	...
	if (IMPP_EXT_USERBUFFER == ctx->src_buf_type) {
		_h264_dec_init_exit_buffers(&pSrc[index], width, height,
...
static int h264_dec_dstbuffer_get(void* chn_ctx,IMPP_BufferInfo_t *buffer,int index)
{
	/* only support userptr */
	assert(chn_ctx);
	h264d_chn_ctx_t *ctx = (h264d_chn_ctx_t *)chn_ctx;
...
static int h264_dec_dstbuffer_set(void* chn_ctx,IMPP_BufferInfo_t *buffer,int index)
{
	/* only support userptr */
	assert(chn_ctx);
	h264d_chn_ctx_t *ctx = (h264d_chn_ctx_t *)chn_ctx;
	...
	if (IMPP_EXT_USERBUFFER == ctx->dst_buf_type) {
		_h264_dec_init_exit_buffers(&pDst[index], width, height,
...
static int h264_dec_poll_srcbuffer_avaliable(void* chn_ctx, int wait_type)
{
	assert(chn_ctx);
	int ret = 0;
	h264d_chn_ctx_t *ctx = (h264d_chn_ctx_t *)chn_ctx;
...
int h264_dec_poll_dstbuffer_avaliable(void* chn_ctx, int wait_type)
{
...
static int h264_dec_qSrc(void* chn_ctx, IMPP_BufferInfo_t *bufinfo)
{
	assert(chn_ctx);
	h264d_chn_ctx_t *ctx = (h264d_chn_ctx_t *)chn_ctx;
...
static int h264_dec_dqSrc(void* chn_ctx, IMPP_BufferInfo_t *bufinfo)
{
	assert(chn_ctx);
	h264d_chn_ctx_t *ctx = (h264d_chn_ctx_t *)chn_ctx;
	int fd = ctx->fd;
...
static int h264_dec_qDst(void* chn_ctx, IHAL_CodecStreamInfo_t *bufinfo)
{
	int ret = 0;
	int index = bufinfo->index;
	assert(chn_ctx);
	...
	ret = _h264_dec_qDst(chn_ctx, index);
	if (ret)
		return -IHAL_RERR;
...
static int h264_dec_dqDst(void* chn_ctx, IHAL_CodecStreamInfo_t *outbuf)
{
	assert(chn_ctx);
	h264d_chn_ctx_t *ctx = (h264d_chn_ctx_t *)chn_ctx;
	int fd = ctx->fd;
...
static int h264_dec_control(void* chn_ctx, IHal_CodecControlParam_t *param)
{
        assert(chn_ctx);
        assert(param);
        h264d_chn_ctx_t *ctx = (h264d_chn_ctx_t *)chn_ctx;
...
ImppCodec_t h264dec_v1 = {
	...
	.codec_chn_ctx_create = h264_dec_chn_ctx_create,
	.codec_chn_ctx_destroy = h264_dec_chn_ctx_destroy,
	.codec_param_set = h264_dec_param_set,
	.codec_param_get = h264_dec_param_get,
	.codec_chn_start = h264_dec_chn_start,
	.codec_chn_stop = h264_dec_chn_stop,

	.src_buffer_create = h264_dec_srcbuffer_create,
	.src_buffer_free = h264_dec_srcbuffer_free,
	.dst_buffer_create = h264_dec_dstbuffer_create,
	.dst_buffer_free = h264_dec_dstbuffer_free,

	.get_src_buffer = h264_dec_srcbuffer_get,
	.set_src_buffer = h264_dec_srcbuffer_set,
	.get_dst_buffer = h264_dec_dstbuffer_get,
	.set_dst_buffer = h264_dec_dstbuffer_set,

	.poll_src_buffer_avaliable = h264_dec_poll_srcbuffer_avaliable,
	.poll_dst_buffer_avaliable = h264_dec_poll_dstbuffer_avaliable,

	.queue_src_buffer = h264_dec_qSrc,
	.dequeue_src_buffer = h264_dec_dqSrc,
	.queue_dst_buffer = h264_dec_qDst,
	.dequeue_dst_buffer = h264_dec_dqDst,

	.control = h264_dec_control,
};
