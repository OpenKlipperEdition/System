...
{
        int ret = 0;
        memset((void *)&input_params, 0, sizeof(struct configInfo));
...
static void signal_handle(int sig)
{
        ...
        ret = pthread_cancel(render_tid);
        assert(0 == ret);
        ret = pthread_join(render_tid, NULL);
        assert(0 == ret);
...
int main(int argc, char **argv)
{
        ...
        for (i = 0; i < input_params.nbuffers; i++) { // 3 是最大FrameBuffer数量，也是最大codec可用的数量.
                ...
                printf("=====set buffer: %d, fb0_buf.vaddr: %x\n", i, fb0_buf.vaddr);
        }
...
//	pthread_create(&render_tid, NULL, render_thread, (void *)codec_handle);
        ret = pthread_create(&render_tid, NULL, render_thread, (void *)fb0_handle);
        assert(0 == ret);
