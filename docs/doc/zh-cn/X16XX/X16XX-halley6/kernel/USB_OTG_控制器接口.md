...
设备树默认编译会产生otg 和otg_phy设备，在halley6_v10.dts中配置如下：
```c
&otg {
...
或者可在halley6_v10.dts中配置USB控制器模式：
```c
&otg {
...
hally6_v10 4.4.94内核默认不配置使用省电模式。
...
省电模式需要在板级设备树同时配置id-dete-gpio和vbus-dete-gpio，halley6_v10硬件上没有引出这两根管脚，如果需要使用省电功能，需引出id-dete-gpio和vbus-dete-gpio，并在halley6_v10.dts设备树中配置管脚属性，配置方法可参考halley5_v30.dts。
...
X1600 USB IP遵循USB2.0规范，支持多种测试模式，包括Test_J模式、Test_K模式、Test_SE0_NAK 模式、Test_Packet模式和Test_Force_Enable模式。

1. 作为device进行眼图测试，需要利用电脑端测试软件工具 HSETT，使其进入test_Packet测试模式。

2. 作为host进行眼图测试，需要配置寄存器HPRT[16,13]位，进入test_Packet测试模式。

x1600 usb Phy的IP厂商预留下表可用于调整眼图以获取最佳信号质量，usb phy寄存器映射到总线基地址为0x10078000。


|  |  |  |  |
| --- | --- | --- | --- |
| Addr | Bit region | Default | Description |
| 0x30 | [2:0] | 3'b001 | Enables pre-emphasis under certain circumstances in order tocompensate for excessive capacitive loading on D+ / D-. Thistends to increase the slew rate.3’b001: enable pre-emphasis during SOF and EOP3’b010: enable pre-emphasis during chirp3’b100: enable pre-emphasis in non-chirp state3’b111: always enable pre-emphasis All other combinations are reserved |
| 0x40 | [5:3] | 3’b001 | Tx HS pre_emphasize strength configure, 3’b111 represents the strongest, 3’b000 the weakest |
| 0x11C | [4:0] | 5’b10101 | 45ohm HS ODT value tuning & FS/LS driver strength tuning5’b11111: smallest HS ODT value & largest FS/LS driver strength and fastest FS/LS slew rate.5’b10000: biggest HS ODT value & smallest FS/LS driverstrength and slowest FS/LS slew rate. |
| 0x124 | [4:2] | 3’b000 | HS eye height tuning3’b000 : 400mv(default)3’b001 : 475mv3’b010 : 350mv3’b011 : 500mv3’b100 : 412.5mv3’b101 : 425mv3’b110 : 437.5mv3’b111 : 450mv |
