...
int input_run(int id)
{
    ...
    in->_videosize = pctx->videoIn->framesizeIn;
    in->_userdata = malloc(pctx->videoIn->framesizeIn);
    if(in->buf == NULL) {
...
void *cam_thread(void *arg)
{
    ...
    while(!pglobal->stop) {
		...
		memcpy(pglobal->in[pcontext->id]._userdata, pcontext->videoIn->framebuffer, pcontext->videoIn->framesizeIn);
