...
CrossPROGRAM=$1
DebuggerPath=$(which $(DEVICE_COMPILER_PREFIX)-gdb)
...
echo "         	 \"c\": \" cd $TOPDIR && make $PROJECTNAME && cd $TOPDIR/$PROGRAMPATH && cp $CrossPROGRAM $CrossPROGRAM.old && $(DEVICE_COMPILER_PREFIX)-strip $CrossPROGRAM && adb push $CrossPROGRAM / && mv $CrossPROGRAM.old $CrossPROGRAM && adb shell /gdbserver 127.0.0.1:5000 /$CrossPROGRAM \"," >> $WORKDIR/$FILENAME.code-workspace
echo "         	 \"cpp\": \" cd $TOPDIR && make $PROJECTNAME && cd $TOPDIR/$PROGRAMPATH && cp $CrossPROGRAM $CrossPROGRAM.old && $(DEVICE_COMPILER_PREFIX)-strip $CrossPROGRAM && adb push $CrossPROGRAM / && mv $CrossPROGRAM.old $CrossPROGRAM && adb shell /gdbserver 127.0.0.1:5000 /$CrossPROGRAM \"," >> $WORKDIR/$FILENAME.code-workspace
echo "        }" >> $WORKDIR/$FILENAME.code-workspace
echo "    }" >> $WORKDIR/$FILENAME.code-workspace
echo "}" >> $WORKDIR/$FILENAME.code-workspace
