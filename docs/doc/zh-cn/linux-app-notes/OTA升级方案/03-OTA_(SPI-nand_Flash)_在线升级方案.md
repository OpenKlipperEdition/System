# **03-OTA (SPI-nand Flash) 在线升级方案**

# 【1】说明<a id="section1"></a>

1. 本文档编辑时间：2024-11-18
2. OTA升级过程以X2000 Halley5 NAND为例进行
3. SDK中的目录以X2000 Halley5的SDK环境为例，其他SDK环境依据这个进行参考
...
​	在[《01_OTA简介与docker服务器搭建.md》](./01_OTA简介与docker服务器搭建.md)中介绍了一个通过nginx+python实现的https服务器，该服务器使用docker方式搭建，如果需要可以参考。在服务器搭建完成之后，需要在指定的OTA升级目录（以上述搭建服务器为例，该目录为`$DOCK_SERVER_PATH/ota`）下存放一个`current_version_full.conf`文件，在这个文件中保存着OTA升级包在服务器中的存放位置，可以参考`device/x2000halley5/ota-overlay/package_config/current_version.conf`文件示例进行修改，示例如下：
...
​	对于下载的SDK，首先需要在SDK顶层目录下执行`source build/envsetup.sh`配置环境变量，然后在执行`lunch`，如下（以Halley5 V30开发板为例，基于NAND介质的OTA升级为例）：

![03-OTA_(SPI-nand_Flash)_在线升级方案.18](./assets/03-OTA_(SPI-nand_Flash)_在线升级方案.18.png)

​	修改`device/x2000halley5/ota-overlay/package_config/recovery.conf`文件来配置服务器中`current_version_full.conf`文件的存放位置，以[【3】OTA服务器的搭建](#section3)中配置的环境为例，`recovery.conf`内容如下：
...
```c
test@user:~/release/x2000$ ll out/product/x2000halley5.v30_nand_6.6_ota-eng/image/
total 64096
drwxr-xr-x 2 test sw     4096 Nov 18 15:47 ./
drwxr-xr-x 4 test sw     4096 Nov 18 15:44 ../
-rw-r--r-- 1 test sw  6058048 Nov 18 15:45 kernel
-rw-r--r-- 1 test sw  9781312 Nov 18 15:46 kernel_recovery
-rw-r--r-- 1 test sw 49393664 Nov 18 15:47 system.ubifs
-rw-r--r-- 1 test sw   381360 Nov 18 15:47 uboot
```

​	执行`cat out/product/x2000halley5.v30_nand_6.6_ota-eng/system/etc/ota_res/recovery.conf`验证配置的recovery是否正确，如果正确则将编译出来的镜像拷贝到PC上，用于后续的烧录操作。

# 【5】OTA升级包制作<a id="section5"></a>

## 【5.1】修改分区表信息配置<a id="section5-1"></a>

​	修改`device/x2000halley5/ota-overlay/package_config/partition_nand.conf`文件配置NAND的大小和分区情况，具体的分区情况应与烧录工具保持一致（即与产品NAND的实际分区情况保持一致）：

![图1 烧录工具分区信息](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.0.png)

![图2 分区情况](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.1.png)

## 【5.2】修改升级信息配置<a id="section5-2"></a>

​	修改`device/x2000halley5/ota-overlay/package_config/customization_nand.conf`文件配置升级信息，参考如下：

![](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.2.png)
...
​	因为编译生成kernel_recovery时需要ramdisk，ramdisk的制作是根据`ramdisk_nand.conf`文件（以NAND为例，文件位置为：`device/x2000halley5/ota-overlay/ramdisk_config/ramdisk_nand.conf`）中列出的文件列表信息从`out/product/x2000halley5.v30_nand_6.6_ota-eng/obj/buildroot-intermediate/target`目录（即`target`目录）下进行抽取的。如果需要修改ramdisk中的文件，则可以通过修改`ramdisk_nand.conf`文件进行添加和删除。

​	下面看一下`ramdisk_nand.conf`文件的内容：

![](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.11.png)

​	每一行的内容都是文件在`target`目录下的相对路径，以`./lib/firmware`和`./etc/init.d/S21mount`为例（参见下图），因为`./lib/firmware`是一个目录，其在制作ramdisk的时候会将这个目录抽取出来，而`./etc/init.d/S21mount`是一个文件，则只会抽取这一个文件，若当前行以`#`开头，则不进行抽取。

![](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.12.png)

​	如果想要验证抽取的ramdisk是否是自己所需要的，可以在编译完kernel_recovery之后到`out/product/x2000halley5.v30_nand_6.6_ota-eng/obj/kernel_recovery-intermediate/ramdisk`目录（根据实际情况修改路径）下进行查看，如下图：

![](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.13.png)

#### 【5.3.1.2】kernel_recovery的defconfig配置<a id="section5-3-1-2"></a>

​	以X2000 Halley5 NAND OTA升级为例，其所需要的默认配置文件是`halley5_v30_linux_sfc_nand_recovery_defconfig`文件，具体开发板所对应的是哪个配置可参见`device/x2000halley5/kernel-recovery.mk`文件（以halley5开发板为例），如下图所示：

![](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.14.png)
...
​	在SDK顶层目录下执行`make kernel_recovery`命令单独编译kernel_recovery，执行`make`命令时进行整体编译的过程中会编译kernel_recovery，生成的镜像文件存储在`out/product/x2000halley5.v30_nand_6.6_ota-eng/image/`目录下。

​	如果生成的kernel_recovery不能启动到ramdisk环境，请转到[【8】OTA升级测试常见问题](#section8)。

### 【5.3.2】uboot升级镜像文件制作<a id="section5-3-2"></a>

​	和uboot编译相关的文件可参考`device/x2000halley5/uboot.mk`，对于Halley5 V30 Nand介质的开发板其对应的config为`halley5_xImage_sfc_nand_ota`，如下图所示：

![03-OTA_(SPI-nand_Flash)_在线升级方案.19](./assets/03-OTA_(SPI-nand_Flash)_在线升级方案.19.png)

​	需要注意`halley5_uImage_sfc_nand`和`halley5_xImage_sfc_nand_ota`的主要区别是`halley5_xImage_sfc_nand_ota`增加了`SPL_OS_BOOT`和`OTA_VERSION30`选项（Option）。

​	在SDK顶层目录下执行`make uboot`可单独编译uboot，生成的文件在`out/product/x2000halley5.v30_nand_6.6_ota-eng/image/`目录下，然后将生成的uboot镜像文件拷贝到本地PC上用烧录工具**合成镜像并填充分区表信息，必须确保与产品分区保持一致**，其操作如下：

![图1 烧录工具分区信息](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.3.png)

![](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.4.png)

​	然后在将要编译生成OTA升级包之前将生成的image.bin拷贝到`out/product/x2000halley5.v30_nand_6.6_ota-eng/image`目录下（根据实际情况进行修改），并重新命名为uboot。

### 【5.3.3】kernel升级镜像文件制作<a id="section5-3-3"></a>

​	和产品的kernel制作方法一致，生成的镜像文件在`out/product/x2000halley5.v30_nand_6.6_ota-eng/image`目录下。

### 【5.3.4】rootfs升级镜像文件制作<a id="section5-3-4"></a>

#### 【5.3.4.1】buildroot的defconfig配置<a id="section5-3-4-1"></a>

​	X2000 Halley5 OTA升级时buildroot的默认配置文件是`halley5_linux_ota_defconfig`，具体开发板对应的是哪个配置可参见`device/x2000halley5/device.mk`文件（以halley5开发板为例），如下图所示：

![](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.9.png)
...
​	在`device/x2000halley5`目录下的`rootfs-overlay`用于存储最基本的一些文件供开发板使用，对于NAND OTA升级时所需要的文件则存储在`device/x2000halley5/ota-overlay/rootfs_nand_overlay`目录下，比如挂载usrdate分区、拷贝`/etc/ota_res/`到`/usr/data/`目录下等相关的脚本文件等：

![](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.10.png)

## 【5.4】编译生成升级镜像包<a id="section5-4"></a>

​	上述镜像文件制作完成之后都在`out/product/x2000halley5.v30_nand_6.6_ota-eng/image`目录下，如下：

```
test@user:~/release/x2000/out/product/x2000halley5.v30_nand_6.6_ota-eng/image$ ls -l
total 64088
-rw-r--r-- 1 test sw  6058048 Nov 18 15:45 kernel
-rw-r--r-- 1 test sw  9781312 Nov 18 16:47 kernel_recovery
-rw-r--r-- 1 test sw 49393664 Nov 18 15:47 system.ubifs
-rw-r--r-- 1 test sw   381360 Nov 18 17:26 uboot
```

​	需要注意此处的uboot镜像文件是在PC上使用烧录工具填充分区表信息之后再拷贝到服务器上的（详见[【5.3.2】uboot升级镜像文件制作](#section5-3-2)）。然后在SDK顶层目录下执行`make ota_mkpackage`命令来制作OTA升级镜像包，其结果如下图所示：

![](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.5.png)

![](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.6.png)
...
​	然后在服务器存放升级包的位置处执行`mkdir full ; 7zr x ota-package.7z -ofull`命令解压升级包，解压后服务器存放升级包的目录下内容如下所示：
...
![](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.7.png)
...
​			修改`buildroot/package/wpa_supplicant/wpa_supplicant.conf`文件来配置要连接的WIFI或者在`device/x2000halley5/rootfs-overlay/etc`目录下添加`wpa_supplicant.conf`文件
...
![](assets/03-OTA_(SPI-nand_Flash)_在线升级方案.8.png)
