...
SSI是一个全双工同步串行接口，可以连接到多种外部模拟-数字（A/D）转换器、音频和电信编解码器以及其他使用串行传输数据的协议。X2500支持SSI摩托罗拉的串行外设接口（SPI）协议。
...
SSI0 PC2-5 FUNCTION1  
SSI1 PB25-30 FUNCTION2  
SSI1 PC8-13 FUNCTION2
...
/ {
 ...
 gpio-sck = <&gpb 8 GPIO_ACTIVE_LOW INGENIC_GPIO_NOBIAS>;

 gpio-miso = <&gpb 15 GPIO_ACTIVE_LOW INGENIC_GPIO_NOBIAS>;

 gpio-mosi = <&gpb 16 GPIO_ACTIVE_LOW INGENIC_GPIO_NOBIAS>;

 cs-gpios = <&gpb 11 GPIO_ACTIVE_LOW INGENIC_GPIO_NOBIAS>;
...
/ {
 ...
 sck-gpios = <&gpb 8 GPIO_ACTIVE_HIGH INGENIC_GPIO_NOBIAS>;

 miso-gpios = <&gpb 15 GPIO_ACTIVE_HIGH INGENIC_GPIO_NOBIAS>;

 mosi-gpios = <&gpb 16 GPIO_ACTIVE_HIGH INGENIC_GPIO_NOBIAS>;

 cs-gpios = <&gpb 11 GPIO_ACTIVE_HIGH INGENIC_GPIO_NOBIAS>;
...
针对spi接口的nor flash，内核已经有对应驱动支持，如果想使用只需要正确配置即可，在使用spi接口的nor flash驱动时应采用gpio模拟spi协议的方式。
...
module_drivers/dts/hippo_v12.dts
