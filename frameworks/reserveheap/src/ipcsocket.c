...
/**
 * @brief opensocket
 * @details 打开socket用于dmabuf的文件句柄通讯.
 * @param[out] 打开socket传出的sockfd句柄
 * @param[in] 用于通讯的文件名，路径需要在/tmp中
 * @param[in] connecttype 1 表示服务端或导出端，0表示客户端或导入端
 * @return 0: 表示成功，其他失败
 */
...
/**
 * @brief sendtosocket
 * @details 发送socket数据和文件句柄
 * @param[in] sockfd opensocket打开的socketfd句柄
 * @param[out] skdata 发送的数据结构体
 * @return 0: 成功 其他失败
 */
...
int sendtosocket(int sockfd, struct socketdata *skdata)
{
	...
	if (FD_ISSET(sockfd, &selFDs)) {
		...
		iov.iov_base = skdata->data;
		iov.iov_len = sizeof(skdata->data);
		msgh.msg_iov = &iov;
		msgh.msg_iovlen = 1;
...
/**
 * @brief receivefromsocket
 * @details 接收数据和文件句柄
 * @param[in] sockfd opensocket打开的socket句柄
 * @param[out] skdata 接收的sockdata结构
 * @return 0: 成功 其他失败
 */
...
/**
 * @brief closesocket
 * @details 关闭socket句柄，并删除open创建的文件
 * @param[in] sockfd socketfd句柄
 * @param[in] name 文件名称
 * @return 0: 成功
 */
