...
#include "impp.h"
#include "ffplay.h"
extern IMPP_BufferInfo_t fb_buf[FFPLAY_FB_BUFFER_COUNT];
#endif
...
int ff_v4l2_buffer_initialize(V4L2Buffer* avbuf, int index)
{
    ...
    int ret, i;
#ifdef FRAMEBUF_THROUGH
    /* The following is the modification by swordslee to modify the v4l2 decoder to achieve Zero-copy */
    if (ctx->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE || ctx->type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
    ...
    for (i = 0; i < avbuf->num_planes; i++) {
        ...
        if (V4L2_TYPE_IS_MULTIPLANAR(ctx->type)) {
            ...
            if (ctx->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE || ctx->type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
                ...
                avbuf->plane_info[i].mm_addr = (void*)(fb_buf[index].vaddr + i * ctx->width * ctx->height);
                avbuf->buf.m.planes[i].m.userptr = (unsigned long)avbuf->plane_info[i].mm_addr;
                avbuf->buf.m.planes[i].length = avbuf->buf.m.planes[i].length;
                avbuf->buf.m.planes[i].bytesused = 0;
#else
                avbuf->plane_info[i].length = avbuf->buf.m.planes[i].length;
