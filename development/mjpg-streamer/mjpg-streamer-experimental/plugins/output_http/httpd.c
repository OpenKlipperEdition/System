...
pthread_mutex_t  disparity_mutex;
pthread_cond_t  disparity_update;
...
/* TODO: this function need fixed!! */
int _readcontext(int fd, iobuffer *iobuf, void *buffer, size_t len, int timeout)
{
    char c = '\0', *out = buffer;
    int i;
...
Return Value: -
******************************************************************************/
void send_capture(cfd *context_fd, int input_number)
{
    unsigned char *frame = NULL;
    int frame_size = 0;
    char buffer[BUFFER_SIZE] = {0};
    ...
    // 调用线程, 计算 disp
    int ret = compute_stereo_disparity(pglobal, input_number);

    /* copy v4l2_buffer timeval to user space */
    timestamp = pglobal->in[input_number].timestamp;

    // TODO: need return rectify picture as mjpg.
    memcpy(frame, pglobal->in[input_number].buf, frame_size);
    DBG("got frame (size: %d kB) _userdata %d kB\n",
	frame_size / 1024, pglobal->in[input_number]._videosize / 1024);
...
Return Value: -
******************************************************************************/
void send_compute_disparity(cfd *context_fd, int input_number, iobuffer *iobuf)
{
    unsigned char *frame = NULL;
    int frame_size = 0;
    char buffer[BUFFER_SIZE] = {0};
    struct timeval timestamp;
    int fd = context_fd->fd;
    int cnt;
    int i;
    cnt = _readcontext(fd, iobuf, buffer, sizeof(buffer) - 1, 0);
    if (strlen(buffer) < 1)
    {
      DBG("get JSON file fail\n");
      sprintf(buffer, "HTTP/1.0 404 Not Found\r\n"
	      "Content-type: text/plain\r\n"
	      STD_HEADER
	      "\r\n"
	      "404: Not Found!\r\n"
	      "%s", "get coordinate fail");
	     goto ending;
    }

    cJSON *root,*x1,*y1,*x2,*y2;
    root = cJSON_Parse(buffer);
    x1 = cJSON_GetObjectItem( root , "x1" );
    y1 = cJSON_GetObjectItem( root , "y1" );
    x2 = cJSON_GetObjectItem( root , "x2" );
    y2 = cJSON_GetObjectItem( root , "y2" );

    _coordinate point1;
    _coordinate point2;
    point1.pixel_x = x1->valueint;
    point1.pixel_y = y1->valueint;
    point2.pixel_x = x2->valueint;
    point2.pixel_y = y2->valueint;

    // get disparity mutex, if disparity donnt gotten,
    // condithon disparity
    sprintf(buffer, "HTTP/1.0 200 OK\r\n"		\
            "Content-type: %s\r\n"			\
            STD_HEADER					\
    ...
    /* maybe need while (disparity had be computed) */
    /* pthread_cond_wait(&disparity_update, &disparity_mutex); */
    /* fprintf(stderr, "%s %d \n",__FUNCTION__, __LINE__); */

    _coordinate central;
    compute_depth(&point1,  &point2, &central);
    ...
    sprintf(buffer + strlen(buffer),
	    ...
	    "\"point1_disp\": %d"
	    "}",
	    central.world_x, central.world_y, central.world_z, central.disp);
...
/******************************************************************************
Description.: Send a  param to client.
Input Value.: fildescriptor fd to send the answer to
Return Value: -
******************************************************************************/
void send_param(cfd *context_fd, int input_number, iobuffer *iobuf)
{
    char buffer[BUFFER_SIZE] = {0};
    char buff_path[BUFFER_SIZE] = {0};
    FILE *f;
    int fd = context_fd->fd;
    int id = (context_fd->pc)->id;
    int cnt;
    int len, i;
    char *data;

    config conf = servers[id].conf;
    char *filepath = conf.www_folder;

    cnt = _readline(fd, iobuf, buffer, sizeof(buffer) - 1, 0);
    ...
    if(strlen(buffer) < 1) {
        ...
        sprintf(buffer, "HTTP/1.0 404 Not Found\r\n"
                ...
                "\r\n"
                "404: Not Found!\r\n"
                "%s", "get  param file path fail");
        goto ending;
    }

    cJSON *root, *json_name;
    root = cJSON_Parse(buffer);
    json_name = cJSON_GetObjectItem(root, "filename");
    memcpy(buff_path, filepath, strlen(filepath));
    sprintf(buff_path + strlen(buff_path), "%s", json_name->valuestring);


    f = fopen(buff_path, "rb");
    if(f != NULL) {
        fseek(f, 0, SEEK_END);
        len = ftell(f);
        fseek(f, 0, SEEK_SET);
        ...
        sprintf(buffer, "HTTP/1.0 404 Not Found\r\n"
                ...
                STD_HEADER
                "\r\n"
                "404:  PARAM JSON File Not Found!\r\n"
                "%s", "get  param json file fail");
        goto ending;
    }
    ...
    sprintf(buffer + strlen(buffer), "%s", data);
ending:
    i = strlen(buffer);
    if(write(context_fd->fd, buffer, i) < 0) {
        DBG("unable to serve the control JSON file\n");
    }
}

/******************************************************************************
Description.: Save  param From client.
Input Value.: fildescriptor fd to send the answer to
Return Value: -
******************************************************************************/
void save_param(cfd *context_fd, int input_number, iobuffer *iobuf)
{
    char buffer[BUFFER_SIZE] = {0};
    char buff_path[BUFFER_SIZE] = {0};
    int fd = context_fd->fd;
    ...
    if(strcmp(json_name->valuestring, SGBM_CUS_JSON) == 0){
        setup_sgbm(buffer);
    } else if(strcmp(json_name->valuestring, CALIB_CUS_JSON) == 0){
        setup_calib(buffer);
    }
...
/******************************************************************************
Description.: update disparity to client.
Input Value.: fildescriptor fd to send the answer to
Return Value: -
******************************************************************************/
void update_disp(cfd *context_fd, int input_number)
{
    char buffer[BUFFER_SIZE] = {0};
    unsigned char *frame = NULL;
    int frame_size = 0;
    ...
    if(write(context_fd->fd, buffer, strlen(buffer)) < 0) {
        DBG("unable to serve update disp\n");
    }
    free(frame);
}

/******************************************************************************
Description.: Send disp jpeg to client.
Input Value.: fildescriptor fd to send the answer to
Return Value: -
******************************************************************************/
void send_disparity(cfd *context_fd, int input_number)
{
    char buffer[BUFFER_SIZE] = {0};
    unsigned char * frame = NULL;
    int frame_size = 0;
    ...
    frame_size = disparity_jpeg_compress(&frame);

    OPRINT("disp size is %d \n",frame_size);
    if(frame_size != 0) {
        sprintf(buffer, "HTTP/1.0 200 OK\r\n" \
                "Access-Control-Allow-Origin: *\r\n" \
                STD_HEADER \
        ...
        sprintf(buffer, "HTTP/1.0 404 Not Found\r\n"
                ...
                STD_HEADER
                "\r\n"
                "404: NOT FOUND DISPARITY!\r\n"
                "%s", "get disparity frame fail");
    }

    /* send header and image now */
    if ((write(context_fd->fd, buffer, strlen(buffer)) < 0)
	|| (write(context_fd->fd, frame, frame_size) < 0)) {
        DBG("unable to send disparity.\n");
    }
    free(frame);
}
...
Return Value: -
******************************************************************************/
void send_calib_select(cfd *context_fd, int input_number)
{
     char buffer[BUFFER_SIZE] = {0};
     ...
     // get corner point and insert to stereo_calib vector
     pthread_t thread_id = calibrate_get_corner_point(pglobal, input_number);
     if (thread_id == -1){
         pthread_mutex_unlock(&pglobal->in[input_number].db);
         send_error(context_fd->fd, 500, "get corner point fail");
         return;
     }
     ...
     void *thread_retval;
     if (pthread_join(thread_id, &thread_retval) != 0) {
         send_error(context_fd->fd, 500, "get corner point fail");
         return;
     }
     /* write the response */
    sprintf(buffer, "HTTP/1.0 200 OK\r\n"		\
     ...
     /* send message to web */
     if ((long)thread_retval != 0) {
         OPRINT("error pattern \n");
         sprintf(buffer + strlen(buffer), "{ \"pattern\" : \"error\" }");
     } else {
         OPRINT("good pattern\n");
         sprintf(buffer + strlen(buffer), "{ \"pattern\" : \"good\" }");
     }

     /* send header now */
     if (write(context_fd->fd, buffer, strlen(buffer)) < 0) {
       DBG("unable to send selected image.\n");
     }
     return;
}

void send_env_check(cfd *context_fd, int input_number)
{
    char buffer[BUFFER_SIZE] = {0};
    sprintf(buffer, "HTTP/1.0 200 OK\r\n"		\
            "Content-type: %s\r\n"			\
    ...
    int env_isvalid = get_env_flag();
    if(env_isvalid) {
        sprintf(buffer + strlen(buffer), "{ \"env_isvalid\" : \"YES\" }");
    } else {
        sprintf(buffer + strlen(buffer), "{ \"env_isvalid\" : \"NO\" }");
    }
...
void send_calib_clean(cfd *context_fd, int input_number)
{
     unsigned char *frame = NULL;
     int frame_size = 0;
     char buffer[BUFFER_SIZE] = {0};
     ...
     int ret = calibrate_clean();
     OPRINT("clear calibrate vector \n");
...
void send_calibrate(cfd *context_fd, int input_number)
{
     char buffer[BUFFER_SIZE] = {0};
     char *pb , *pstart;
     int ret = calibrate();
     ...
     get_calibrate_rms(&lrms, &rrms, &stereorms);

     get_calibrate_Q(&buffer);
     int sizeQ = strlen(buffer);
     pb = buffer + strlen(buffer);
     get_calibrate_T(pb);
     int sizeMat = strlen(buffer);
     int sizeT = sizeMat - sizeQ;
     pstart = buffer + sizeMat;

     sprintf(buffer + strlen(buffer), "HTTP/1.0 200 OK\r\n"				\
	     "Content-type: text/plain\r\n"				\
	     STD_HEADER							\
	     "\r\n");
     ...
     if (ret == 0) {
         sprintf(buffer + strlen(buffer),
                 ...
                 "\"MatT\" : %.*s"
                 "}",
                 lrms, rrms, stereorms, sizeQ, buffer, sizeT, pb);
         OPRINT("calibrate success\n");
     } else {
         sprintf(buffer + strlen(buffer), "{\"calibrate\" : \"fail\" }");
         OPRINT("calibrate false\n");
     }


     if (write(context_fd->fd, pstart, (strlen(buffer) - sizeMat)) < 0) {
         DBG("unable to serve the control JSON file\n");
     }
     return;
}

void send_calib_apply(cfd *context_fd, int input_number)
{
     unsigned char *frame = NULL;
     int frame_size = 0;
     char buffer[BUFFER_SIZE] = {0};
     ...
     if (ret == 0) {
         sprintf(buffer + strlen(buffer), "{\"calibrate_apply\" : \"success\" }");
         OPRINT("calibrate apply success\n");
     } else {
         sprintf(buffer + strlen(buffer), "{\"calibrate_apply\" : \"fail\" }");
         OPRINT("calibrate apply false\n");
     }
...
void send_calib_rectify_check(cfd *context_fd, int input_number)
{
     unsigned char *frame = NULL;
     int frame_size = 0;
     char buffer[BUFFER_SIZE] = {0};
     ...
     OPRINT("check rectify \n");
     // get corner point and insert to stereo_calib vector
     int ret = calibrate_check_rectify(pglobal, input_number, &frame, &frame_size);
     if (ret) {
         if (frame)
             free(frame);
     ...
     if (write(context_fd->fd, buffer, strlen(buffer)) < 0 ||
         write(context_fd->fd, frame, frame_size) < 0) {
         DBG("unable to send rectified image.\n");
     }
...
void *client_thread(void *arg)
{
    ...
    if(strstr(buffer, "GET /?action=snapshot") != NULL) {
        ...
        #endif
    } else if(strstr(buffer, "GET /?action=capture") != NULL) {
        req.type = A_CAPTURE;
        query_suffixed = 255;
        #ifdef MANAGMENT
        if (check_client_status(lcfd.client)) {
        ...
        #endif
    } else if(strstr(buffer, "POST /getdepth") != NULL) {
        req.type = A_GETDEPTH;
        query_suffixed = 255;
        #ifdef MANAGMENT
        if (check_client_status(lcfd.client)) {
        ...
        #endif
    } else if(strstr(buffer, "POST /loadparam") != NULL) {
        req.type = A_PARAM_LOAD;
        query_suffixed = 255;
        #ifdef MANAGMENT
        if (check_client_status(lcfd.client)) {
            req.type = A_UNKNOWN;
        ...
        #endif
    } else if(strstr(buffer, "POST /saveparam") != NULL) {
        req.type = A_PARAM_SAVE;
        query_suffixed = 255;
        #ifdef MANAGMENT
        if (check_client_status(lcfd.client)) {
            req.type = A_UNKNOWN;
        ...
        #endif
    } else if(strstr(buffer, "GET /updatedisp") != NULL) {
        req.type = A_DISP_UPDATE;
        query_suffixed = 255;
        #ifdef MANAGMENT
        if (check_client_status(lcfd.client)) {
            req.type = A_UNKNOWN;
        ...
        #endif
    } else if(strstr(buffer, "GET /?action=showdisp") != NULL) {
        req.type = A_DISP_SEND;
        query_suffixed = 255;
        #ifdef MANAGMENT
        if (check_client_status(lcfd.client)) {
            req.type = A_UNKNOWN;
    ...
    #endif
    } else if (strstr(buffer, "POST /calib_clean")) {
	req.type = A_CALIB_CLEAN;
	query_suffixed = 255;
    } else if (strstr(buffer, "POST /env_check")) {
	req.type = A_ENV_CHECK;
	query_suffixed = 255;
    } else if (strstr(buffer, "POST /calib_apply")) {
	req.type = A_CALIB_APPLY;
	query_suffixed = 255;
    } else if (strstr(buffer, "GET /?action=rectify_check")) {
	req.type = A_CALIB_RECTIFY_CHECK;
	query_suffixed = 255;
    } else if (strstr(buffer, "POST /calib_process")) {
	req.type = A_CALIBRATE;
	query_suffixed = 255;
    } else if (strstr(buffer, "POST /calib_select")) {
	req.type = A_CALIB_SELECT;
	query_suffixed = 255;
    ...
    case A_SNAPSHOT:
        ...
        break;
    case A_CAPTURE:
        DBG("Request for capture from input: %d\n", input_number);
        send_capture(&lcfd, input_number);
        break;
    case A_GETDEPTH:
        DBG("Request for capture from input: %d GETDEPTH\n", input_number);
        send_compute_disparity(&lcfd, input_number, &iobuf);
        break;
    case A_STREAM:
        ...
        break;
    case A_PARAM_LOAD:
        DBG("Request for load default  param from input: %d\n", input_number);
        send_param(&lcfd, input_number, &iobuf);
        break;
    case A_PARAM_SAVE:
        DBG("Request save  param from input: %d\n", input_number);
        save_param(&lcfd, input_number, &iobuf);
        break;
    case A_DISP_UPDATE:
        DBG("Request update disparity from input: %d\n", input_number);
        update_disp(&lcfd, input_number);
        break;
    case A_DISP_SEND:
        DBG("Request for disparity from  input: %d\n", input_number);
        send_disparity(&lcfd, input_number);
        break;
    case A_CALIB_SELECT:
        DBG("Request selected calibrate image from input: %d\n", input_number);
        send_calib_select(&lcfd, input_number);
        break;
    case A_CALIB_APPLY:
        DBG("Request calibrate apply from input: %d\n", input_number);
        send_calib_apply(&lcfd, input_number);
        break;
    case A_CALIB_RECTIFY_CHECK:
        DBG("Request rectify check for calibrate from input: %d\n", input_number);
        send_calib_rectify_check(&lcfd, input_number);
        break;
    case A_CALIB_CLEAN:
        DBG("Request selected calibrate clean from input: %d\n", input_number);
        send_calib_clean(&lcfd, input_number);
        break;
    case A_CALIBRATE:
        DBG("Request calibrate from input: %d\n", input_number);
        send_calibrate(&lcfd, input_number);
        break;
    case A_ENV_CHECK:
        DBG("Request env check from input: %d \n", input_number);
        send_env_check(&lcfd, input_number);
        break;
    #ifdef WXP_COMPAT
    case A_STREAM_WXP:
...
void *server_thread(void *arg)
{
    ...
    /* here init mutex */
    if (pthread_mutex_init(&disparity_mutex, NULL))
      {
        perror("Mutex initialization failed");
	exit(EXIT_FAILURE);
      }

    if (pthread_cond_init(&disparity_update, NULL))
      {
    ...
    while(!pglobal->stop) {
        ...
        for(i = 0; i < max_fds + 1; i++) {
            if(pcontext->sd[i] != -1 && FD_ISSET(pcontext->sd[i], &selectfds)) {
                ...
                DBG("create thread to handle client that just established a connection max_fds %d\n", max_fds);
