...
#else
	int num_proc = AudioProcessing::kChunkSizeMs;
	if(sample_rate_hz==8000)
		num_proc *= 2;
	return static_cast<size_t>(
        num_proc * sample_rate_hz / 1000);
#endif
  }
...
class EchoCancellation {
 public:
  ...
  virtual int set_dtd_enable(int enable) = 0;
  virtual int set_dtd_parameters(int num, float ratio, float thd) = 0;
  virtual int set_aec_mode(int aec_mode) = 0;
  virtual int set_suppression_mode(int suppression_mode) = 0;
  virtual int set_far_pow_thd(float far_pow_thd) = 0;
  virtual int set_aec_safe_suppression_value(float safe_sup_value) = 0;
  virtual int set_mu_parameters(float mu_min, float mu_decay) = 0;
  virtual int set_cor_thds(float cor_thd1, float cor_thd2, float cor_thd3, float cor_thd4) = 0;
  virtual int set_log_interval(int log_interval) = 0;
  virtual int set_restrain_parameters(int restrain_band_center, int restrain_band_wide, float restrain_factor) = 0;
  virtual char* get_aec_version() = 0;
