...
*offs is a array of (binder_size_t), and offs_avail is the number of
binder_size_t so we need to use sizeof(binder_size_t) to calc the offs_avail
value

Usually, offs0 is same as buffer, data0 = buffer + sizeof(binder_size_t) *
array_size(offset)
*
*
*/
...
/*
 * buffer for binder write and read, the write_size and read_size will be store
 * into consumed var.
 */
