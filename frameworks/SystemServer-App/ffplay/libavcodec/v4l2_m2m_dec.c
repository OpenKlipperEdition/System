...
static int v4l2_try_start(AVCodecContext *avctx)
{
    ...
    if(V4L2_TYPE_IS_MULTIPLANAR(capture->type)) {
        capture->format.fmt.pix_mp.pixelformat = V4L2_PIX_FMT_JZ420B;
    }else
        capture->format.fmt.pix.pixelformat = V4L2_PIX_FMT_JZ420B;


    ret = ioctl(s->fd, VIDIOC_S_FMT, &capture->format);
	if (ret) {
        av_log(avctx, AV_LOG_WARNING,"set capture fmt set failed.");
		return ret;
	}
...
dequeue:
    ...
    pkt_pts = avpkt.pts;
    av_packet_unref(&avpkt);
    ff_v4l2_context_dequeue_frame(capture, frame);
...
static av_cold int v4l2_decode_init(AVCodecContext *avctx)
{
     ...
     */
    /* output->width = capture->width =(avctx->coded_width+127) & ~127; */
    /* output->height = capture->height = (avctx->coded_height+15) & ~15; */
...
#ifdef FRAMEBUF_THROUGH
      OFFSET(num_capture_buffers), AV_OPT_TYPE_INT, {.i64 = FFPLAY_FB_BUFFER_COUNT}, 2, INT_MAX, FLAGS },
#else
