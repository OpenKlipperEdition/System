...
int del_from_list(struct svcinfo *info)
{
    struct svcinfo *si;
    struct svcinfo *prev = NULL;

    for (si = svclist; si; prev = si, si = si->next) {
        if (strncmp(si->name, info->name, si->len) == 0) {
            if (prev == NULL)
	    {
		svclist = info->next;
		return 0;
...
int del_svc(tIpcThreadInfo *ti, struct svcinfo *info)
{
    struct svcinfo *si;
    struct svcinfo *prev = NULL;

    del_from_list(info);
    binder_cmd_release(ti, info->handle);
    free(info);
