...
class tensor {
 ...
 public:
  ...
  tensor(const tensor &other): m_data(std::move(other.m_data)), m_shape(std::move(other.m_shape)), m_base(std::move(other.m_base))
  {
  ...
  tensor(const std::vector<std::size_t>& shape):m_data(std::make_shared<storage>()),m_shape(shape)
  {
    update_base();
    m_data->resize(size());
  }
  ...
  void dumpShape(){
    std::cout << "\n[";
    if (!m_shape.empty())
      std::cout << m_shape.front();
    for (std::size_t i = 1; i < m_shape.size(); ++i)
      std::cout << "x" << m_shape[i];
    std::cout << "]";
  }
 private:
...
typedef tensor<float, std::allocator<float>> tensor_float;
typedef tensor<uint8_t, std::allocator<uint8_t>> tensor_uint8;
typedef tensor<int16_t, std::allocator<int16_t>> tensor_int16;
typedef tensor<uint16_t, std::allocator<uint16_t>> tensor_uint16;
typedef tensor<int32_t, std::allocator<int32_t>> tensor_int32;
typedef tensor<int8_t, std::allocator<int8_t>> tensor_int8;

class _tensorX{

 public:
#define construct(type)                                 \
  _tensorX(const tensor_##type & x){                    \
    m_data = new tensor_##type(x);                      \
  }                                                     \
  _tensorX(tensor_##type& x){                           \
    m_data = new tensor_##type(x);                      \
  }                                                     \
  tensor_##type getTensor(tensor_##type& dat) const {   \
    tensor_##type *m = (tensor_##type*)m_data;          \
    dat = *m;                                           \
    delete m;                                           \
    return dat;                                         \
  }                                                     \

  tensor_float getTensor() const{
    tensor_float *m = (tensor_float*)m_data;
    tensor_float y(*m);
    delete m;
    return y;
  }
