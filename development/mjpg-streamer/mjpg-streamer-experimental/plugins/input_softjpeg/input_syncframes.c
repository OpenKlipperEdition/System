...
#define SAVEJPEG
#ifdef SAVEJPEG
#define save_jpeg(filename,size, buf)			\
    do{									\
        FILE *fd = fopen(filename, "w+");					\
        if (fd == 0)							\
        fprintf(stderr, "error open file %s %s\n", filename, strerror(errno)); \
...
int sw_jpeg_enc_init(struct encode_config* config, int quality)
{
	return sw_jpeg_encoder_init(config->width,config->height, quality);
}

int sw_jpeg_enc_stop(struct encode_config* config)
{
	return sw_jpeg_encoder_deinit();
}

int sw_jpeg_enc_start(struct encode_config* config)
{
        int size = sw_jpeg_encoder_start(config->input_buf,config->output_buf,config->pixfmt);
        return size;
}
...
int input_init(input_parameter *param, int id)
{
    ...
    int width, height, format, i, length, quality;
	...
	pctx->videoIn->framesizeIn = width * height * 2;
    else
    ...
    if(sw_jpeg_enc_init(pctx->videoIn->encode_config, quality) == -1) {
...
int input_run(int id)
{
    ...
    pthread_setaffinity_np(pctx->threadID, sizeof(cpu_set_t), &mask);
...
void help(void)
{
    ...
    fprintf(stderr,
            ...
            " [-q | --quality ] .....: set quality of JPEG encoding [0-100], default 50\n" \
            ...
            " [-u | --nv12] ........: Use nv12 format, default: MJPEG (uses more cpu power)\n" \
            " [-y | --yuyv  ] ........: Use yuyv format, default: MJPEG (uses more cpu power)\n" \
            " ---------------------------------------------------------------\n");
...
void *sync_frame_thread(void *arg)
{
    ...
    while(!pglobal->stop) {
	    ...
	    if(mono_frame) {
            ...
            pglobal->in[pcontext->id].size = sw_jpeg_enc_start(pcontext->videoIn->encode_config);
...
void sync_freame_cleanup(void *arg)
{
    ...
    if (pctx->videoIn != NULL) {
        ...
        sw_jpeg_enc_stop(pctx->videoIn->encode_config);
        free(pctx->videoIn);
        pctx->videoIn = NULL;
    }
