...
static void draw_buf_rotate_180(lv_disp_drv_t * drv, lv_area_t * area, lv_color_t * color_p, lv_color_t * rot_buf)
{
    lv_coord_t area_w = lv_area_get_width(area);
    lv_coord_t area_h = lv_area_get_height(area);
    uint32_t total = area_w * area_h;
    uint32_t rot_size = drv->hor_res * drv->ver_res;
...
static void draw_buf_rotate(lv_area_t * area, lv_color_t * color_p)
{
    ...
    if(drv->rotated == LV_DISP_ROT_180) {
        ...
        if(rot_buf == NULL) rot_buf = lv_mem_buf_get(drv->hor_res * drv->ver_res * sizeof(lv_color_t));
        draw_buf_rotate_180(drv, area, color_p, rot_buf);
        ...
        lv_coord_t max_row = LV_MIN((lv_coord_t)(drv->hor_res * drv->ver_res / area_w), area_h);
