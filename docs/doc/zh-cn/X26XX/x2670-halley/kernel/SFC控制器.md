...
###  flash读写测试
1. 执行脚本路径: /testsuite/StorageMedia-Test.sh
...
选择1:测试随机大小的数据(需要根据flash剩余空间,计算大小)

选择2:测试指定大小的数据(Kbyte)

4. 例如输入1，flash读写测试开始,连续测试一段时间,如果无错误打印、异常退出，则正常。
...
1.打开cloner中的Config，在INFO菜单下，选择Board为“**x2660_sfc_nor_lpddr3_linux.cfg**”。
