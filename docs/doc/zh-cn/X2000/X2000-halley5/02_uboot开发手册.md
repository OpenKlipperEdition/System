...
Halley5平台u-boot引导流程如下图所示：

![](assets/01-Halley5_uboot开发手册_v2.0.1.png)从上图可看到，分为两种引导方式：
...
当前Halley5平台，决定u-boot配置的文件主要有：

1. **u-boot/boards.cfg**
2. **u-boot/include/configs/x2000_halley5.h**
...
2. ARCH、CPU、Board name、Vendor、SoC：主要用于Makefile编译时选择代码目录，比如u-boot/include/configs目录下有许多配置文件，选择x2000_halley5.h便是通过Board name确定的；

3. **Options：每一种配置之间的主要区别，u-boot会为附加选项中的每个字段前面加上“CONFIG_”前缀并导出为全局可用的宏定义，这些宏定义会进一步决定其他的配置，目前主要是x2000_halley5.h中的配置。**

### 默认配置

Halley5 共支持八种默认编译配置，如下表所示:



|                             |                                                                                                                                                                                                                                                                                                                       |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| x2000_halley5_uImage_msc2         | **该配置支持从sd卡启动linux内核镜像uImage**CONFIG_SPL_JZMMC_SUPPORT：spl中的mmc相关配置CONFIG_ENV_IS_IN_MMC：环境变量存储相关配置CONFIG_GPT_CREATOR：分区相关配置CONFIG_JZ_MMC_MSC2：msc控制器相关配置CONFIG_SPL_PARAMS_FIXER: 改变CPM频率                                                                            |
| x2000_halley5_uImage_msc0         | **该配置支持从emmc启动linux内核镜像uImage**CONFIG_SPL_JZMMC_SUPPORT：spl中的mmc相关配置CONFIG_ENV_IS_IN_MMC：环境变量存储相关配置CONFIG_GPT_CREATOR：分区相关配置CONFIG_JZ_MMC_MSC0：msc控制器相关配置CONFIG_SPL_PARAMS_FIXER: 改变CPM频率                                                                            |
| x2000_halley5_uImage_sfc_nand     | **该配置支持从nand flash启动linux内核镜像uImage**CONFIG_SPL_SFC_NAND：sfc nand flash相关配置CONFIG_MTD_SFCNAND：sfc nand flash驱动编译配置CONFIG_SPL_PARAMS_FIXER: 改变CPM频率                                                                                                                                        |
| x2000_halley5_uImage_sfc_nor      | **该配置支持从nor flash启动linux内核镜像uImage**CONFIG_SPL_SFC_NOR：sfc nor flash相关配置CONFIG_ENV_IS_IN_SFC：环境变量存储相关配置CONFIG_MTD_SFCNOR：sfc nor flash驱动编译相关配置CONFIG_SPL_PARAMS_FIXER: 改变CPM频率                                                                                               |
| x2000_halley5_xImage_sfc_nand_ota | **该配置支持从nand flash启动linux内核镜像xImage**CONFIG_SPL_SFC_NAND：sfc nand flash相关配置CONFIG_MTD_SFCNAND：sfc nand flash驱动编译相关配置CONFIG_SPL_OS_BOOT：spl直接启动linux内核镜像相关配置CONFIG_OTA_VERSION30：ota升级相关配置                                                                               |
| x2000_halley5_xImage_msc2_ota     | **该配置支持从sd卡启动linux内核镜像xImage**CONFIG_SPL_JZMMC_SUPPORT：spl中的mmc相关配置CONFIG_ENV_IS_IN_MMC：环境变量存储相关配置CONFIG_GPT_CREATOR：分区相关配置CONFIG_JZ_MMC_MSC2：msc控制器相关配置CONFIG_SPL_PARAMS_FIXER: 初始化CPM时钟CONFIG_OTA_VERSION30：ota升级相关配置CONFIG_SPL_PARAMS_FIXER: 改变CPM频率 |
| x2000_halley5_xImage_sfc_nand     | **该配置支持从nand flash启动linux内核镜像xImage**CONFIG_SPL_SFC_NAND：sfc nand flash相关配置CONFIG_MTD_SFCNAND：sfc nand flash驱动编译相关配置CONFIG_SPL_OS_BOOT：spl直接启动linux内核镜像相关配置CONFIG_SPL_PARAMS_FIXER: 改变CPM频率                                                                                |
| x2000_halley5_xImage_sfc_nor      | **该配置支持从nor flash启动linux内核镜像xImage**CONFIG_SPL_SFC_NOR：sfc nor flash相关配置CONFIG_MTD_SFCNOR：sfc nor flash驱动编译相关配置CONFIG_SPL_OS_BOOT：spl直接启动linux内核镜像相关配置CONFIG_ENV_IS_IN_SFC：环境变量存储相关配置CONFIG_SPL_PARAMS_FIXER:改变CPM频率                                            |

### 自定义配置

可根据实际需要，在每种板级配置中增删相应的字段或者新添加一个新的板级配置。

下面以添加一个新的板级配置**x2000_halley5_uImage_msc0**为例进行介绍：

1. 在boards.cfg文件中新起一行，按照该文件的格式要求，先确定第一个配置项为x2000_halley5_uImage_msc0，第二到第六个配置项可参考现存的板级配置，一般都是比较固定的；
2. 主要是附加选项的配置，如果对Halley5平台的u-boot整体配置不是很了解的话，可以先参考现存配置，比如这里可以参考x2000_halley5_uImage_msc2，将JZ_MMC_MSC2改为JZ_MMC_MSC0，这样会选择MSC0相关的配置，其他的保持一致即可；
3. 如果想更加细致的修改附加选项，建议先整体阅读一遍该文档，尤其是第二章关于配置的介绍，在了解了Halley5平台的u-boot整体配置后，再进行修改会更加游刃有余。

## x2000_halley5.h配置文件说明

x2000_halley5.h为u-boot中针对halley5平台核心的配置文件，该配置文件主要包含以下内容：
...
2. CPU、DDR频率配置，其中CPU的频率需要与选择的PLL频率成倍数关系，并且最高支持 1.5GHz, 同时DDR的频率也需要与选择的PLL频率成倍数关系
```c
#define CONFIG_SYS_CPU_FREQ 1200000000
...
1. 使用nand flash启动，在boards.cfg文件的**x2000_halley5_uImage_sfc_nand**配置和**x2000_halley5_xImage_sfc_nand**配置中导出了**CONFIG_SPL_SFC_NAND。**
2. 使用nor flash启动，在boards.cfg文件的**x2000_halley5_uImage_sfc_nor**配置中导出了**CONFIG_SPL_SFC_NOR。**
...
#endif
```
3. MSC2配置，如果定义**CONFIG_JZ_MMC_MSC2**，则MSC2相关的配置会被打开：
```c
...
使用sd卡启动，在boards.cfg文件的**x2000_halley5_uImage_msc2**配置中导出了**CONFIG_JZ_MMC_MSC2**，即当前默认使用MSC2配置。

#### 自定义配置

MSC0、MSC1、MSC2每种配置中，均可以自定义如下配置：

1. 数据传输位数，MSC0最多支持8位，MSC1最多支持4位，MSC2最多支持4位
...
在u-boot/board/ingenic/x2000_halley5/partitions.tab中存放有具体的分区信息：
```c
property:
...
```
2. 网口工作模式选择，需要按照实际连接的网络phy芯片进行选择
```c
...
2. 根据实际速度需要，工作模式可选择RGMII或者RMII，其中RGMII支持10/100/1000兆的总线接口速度，RMII支持10兆和100兆的总线接口速度。
...
**如需要在uboot阶段显示log则需要在include/configs/halley6.h打开**
...
该启动镜像对应**x2000_halley5_uImage_sfc_nand**配置。

1. 在工程顶层目录下执行如下命令，导入编译halley5平台所需的环境变量：
```bash
...
**a.如果是halley5 v20开发板，根据需要在以下配置中进行选择：**

1.x2000halley5.v20_nand_4.4.94-eng

7.x2000halley5.v20_nand_5.10-eng

**b.如果是halley5 v30开发板，根据需要在以下配置中进行选择：**

10.x2000halley5.v30_nand_4.4.94-eng

13.x2000halley5.v30_nand_5.10-eng
...
5. 等待编译完成，在`工程目录/out/product/x2000halley5/image`目录下便会生成uboot镜像文件，该镜像即可直接进行烧录。

### 编译SFC Nor启动镜像

该启动镜像对应**x2000_halley5_uImage_sfc_nor**配置。
...
**a.如果是halley5 v20开发板，根据需要在以下配置中进行选择：**
 ...
 2.x2000halley5.v20_nor_4.4.94-eng

 8.x2000halley5.v20_nor_5.10-eng

**b.如果是halley5 v30开发板，根据需要在以下配置中进行选择：**

11.x2000halley5.v30_nor_4.4.94-eng

14.x2000halley5.v30_nor_5.10-eng
...
该启动镜像对应**x2000_halley5_uImage_msc2**配置。
...
**a.如果是halley5 v20开发板，根据需要在以下配置中进行选择：**
 ...
 3.x2000halley5.v20_msc_4.4.94-eng

 9.x2000halley5.v20_msc_5.10-eng

**b.如果是halley5 v30开发板，根据需要在以下配置中进行选择：**

 12.x2000halley5.v30_msc_4.4.94-eng

 15.x2000halley5.v30_msc_5.10-eng
...
执行make xxxxxx -jN命令进行编译，xxxxxx为具体的板级配置名称，需要根据编译的启动 镜像来决定，详细介绍请参考[2.1 boards.cfg文件说明](#_boards.cfg文件说明)。当前共支持4种板级配置，具体如下表所示：



|                               |                                               |
| ----------------------------- | --------------------------------------------- |
| x2000_halley5_uImage_msc2     | 该配置支持从sd卡启动linux内核镜像uImage       |
| x2000_halley5_uImage_sfc_nand | 该配置支持从nand flash启动linux内核镜像uImage |
| x2000_halley5_uImage_sfc_nor  | 该配置支持从nor flash启动linux内核镜像uImage  |
| x2000_halley5_xImage_sfc_nand | 该配置支持从nand flash启动linux内核镜像xImage |

比如，编译从nand flash启动uImage的启动镜像，执行如下命令：
```bash
$ make x2000_halley5_uImage_sfc_nand -jN
```

其中-jN为使用多线程编译，可以加快编译速度，具体的N值需要根据使用的PC决定。

4. 编译好之后，会在u-boot工程顶层目录下生成u-boot-with-spl-mbr-gpt.bin，该文件便是最终生成的镜像文件，可以直接进行烧录。
...
![](assets/01-Halley5_uboot开发手册_v2.0.2.png)![](assets/01-Halley5_uboot开发手册_v2.0.3.png)

对于每一个命令，还可以执行“help 命令名称”获取该命令具体的使用说明，如下图所示：

![](assets/01-Halley5_uboot开发手册_v2.0.4.png)
...
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| printenv   | 打印当前环境变量                                                                                                                                         |
| setenv     | 设置环境变量，格式：setenv name value …，表示将name 变量设置成value值；如果没有这个参数，表示删除该变量                                                  |
| saveenv    |  保存环境变量到存储设备中                                                                                                                                |
| sleep      | 延迟执行，格式：sleepN，可以延迟N秒钟执行                                                                                                                |
...
具体的，我们可以通过在x2000_halley5.h中将启动参数设置给宏定义CONFIG_BOOTARGS。

在x2000_halley5.h中，根据加载内核时挂载的文件系统的不同均有对应的启动参数，它们有一些共有的内容，在x2000_halley5.h中定义如下：
...
1. console=ttyS3,115200表示串口使用/dev/ttyS3这个设备，波特率为115200
...
网络文件系统一般用于开发时调试使用，当前在x2000_halley5.h中的参考启动参数如下：
```c
...
该启动参数对应**x2000_halley5_uImage_sfc_nor**配置，当前在x2000_halley5.h中的启动参数如下：
````c
#define CONFIG_BOOTARGS 
...
### u-boot加载内核挂载ext4文件系统

该启动参数对应**x2000_halley5_uImage_msc2**配置，当前在x2000_halley5.h中的启动参数如下：
```c
#define CONFIG_BOOTARGS 
...
该启动参数对应**x2000_halley5_uImage_sfc_nand**配置，当前在x2000_halley5.h中的启动参数如下：
```c
#define CONFIG_BOOTARGS 
...
1. uboot只支持一个gmac控制器工作，需要在x2000_halley5.h中配置选择的控制器和对应控制器的接口模式：
```c
/* Select GMAC Controller */
...
2. 开发板端配置
 ...
 ```bash
 x2000_halley5# set ipaddr 192.168.4.145

 x2000_halley5# set serverip 192.168.4.146
 ```

 b.将需要的uImage放入PC服务器文件夹下，执行以下命令将uImage下载到内存 0x80800000位置处：
 ```bash
 x2000_halley5# tftp 0x80800000 uImage
 ```
 c.启动uImage：
 ```bash
 x2000_halley5# bootm 0x80800000
 ```
...
```bash
x2000_halley5# loady 0x80800000 115200
...
![](assets/01-Halley5_uboot开发手册_v2.0.5.png)这里按↓键选择ymodem协议，然后回车；

3. 然后会出现如下图所示画面，这里可以选择需要传输的文件，↑↓键进行移动，双敲空格键进入目录，单敲空格键选择文件，回车键确认选择。

我们选择需要传输的kernel镜像：

![](assets/01-Halley5_uboot开发手册_v2.0.6.png)1. 选择完毕并回车确认后，会出现如下进行文件的传输的画面：

![](assets/01-Halley5_uboot开发手册_v2.0.7.png)1. 等待传输完毕，便可以通过bootm命令启动写到内存0x80800000位置处的kernel镜像:
```bash
$ bootm 0x80800000
```
...
3. x2000快速启动时使用，用于生成spl env参数
```
└── spl_params_fixer_x2000_v12.c
```
