...
支持TFT（MIPI-DPI），SLCD（MIPI-DBI type A，B and C），MIPI-DSI(支持4lane)；
...
| ingenic,rst-gpio | 复位引脚：PC02 |
| ingenic,lcd-pwm-gpio | 背光引脚：PB31 |
...
* 配置兼容的屏（compatible）
* 根据不同型号的屏选择控制引脚(pinctrl-0)，不同的引脚设置在module_drivers/dts/ x2500-pinctrl.dtsi
...
struct jzdsi_data jzdsi_pdata = {
 ...
 .video_config.no_of_lanes = 2, /*按照硬件连接填写，支持1、2、3、4lane*/
...
***pinctrl-0=<*** ***&pwm3_pb>; /*按照实际情况配置pwm的gpio*/*** 
...
***pwms=<&pwmz 3 1000000>; /*选择pwm1控制背光，设置period1000000ns*/***
...
### pwm背光驱动配置（PWM_INGENIC）：

Symbol: PWM_INGENIC [=y] 

Type : tristate 

Prompt: Ingenic PWM support 
