    ...
    def cmdline_parse(cls):
       ...
       '''
        ...
        for i in range(1, int(args.imgcnt)+1):
            ...
            bootmode = raw_input(
                'image%d bootmode [must be one in %s]: ' % (
                    i, cls.bootmodes))
            assert bootmode in cls.bootmodes, '''image bootmode '%s'
                            is not in %s''' % (bootmode, cls.bootmodes)
            ...
            imageinfo = image.Image.Imageinfo(
                imgname, 0, imgtype, hex(imgoffset), 0, bootmode, updatemode)
