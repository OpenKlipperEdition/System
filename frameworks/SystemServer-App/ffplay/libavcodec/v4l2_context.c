...
static int v4l2_release_buffers(V4L2Context* ctx)
{
    ...
    /* The following is the modification by swordslee to modify the v4l2 decoder to achieve Zero-copy */
#ifdef FRAMEBUF_THROUGH
    if (ctx->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE || V4L2_BUF_TYPE_VIDEO_CAPTURE) {
...
#else
    {
            req.memory = V4L2_MEMORY_MMAP;
            req.type = ctx->type;
            req.count = 0; /* 0 -> unmaps buffers from the driver */
    }
#endif
    ...
    for (i = 0; i < ctx->num_buffers; i++) {
        ...
        /* The following is the modification by swordslee to modify the v4l2 decoder to achieve Zero-copy */
        if (ctx->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE || ctx->type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
            //printf("<%s>[%s %d]: %p\n", __FILE__, __func__, __LINE__, ctx);
            //printf("<%s>[%s %d]: %p\n", __FILE__, __func__, __LINE__, ctx->name);
            /*av_log(logger(ctx), AV_LOG_DEBUG, "%s: we donot need unmap buf because zero-copy\n", ctx->name);*/
        } else
#endif
       {
