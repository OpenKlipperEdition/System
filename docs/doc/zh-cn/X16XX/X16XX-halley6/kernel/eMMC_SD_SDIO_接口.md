...
x1600使用的ap6212芯片。 AP6212是基于BCM4340A1方案的集成wifi和bluetooth的功能模块，它包括用于WiFi的SDIO接口，和用于蓝牙的UART/PCM接口。
...
| --- | --- | --- |
| **Name** | **I/O** | **Funtion** |
| **SDIO** | GPIO-(PB12~PB17) | FUNTION0 |
| **WL_REG_ON** | GPIO-PC28 |  |
| **WL_WAKE_HOST** | GPIO-PB04 |  |
...
| **MSC0_DATA** | GPIO-(PD02~PD05) | FUNTION0 |
...
在板级设备树halley6_v10.dts中，默认配置产生msc1设备，描述如下：
...
| --- | --- |
| **属性名称** | **说明** |
| **cap-sd-highspeed**| 配置SD支持high speed模式 |
| **max-frequency**| 通过配置该参数来指定当前模式下最大频率 |
| **bus-width**| 配置数据总线宽度，支持1bit、4bit、8bit |
| **voltage-ranges**| 指定电压范围 |
...
首先配置INGENIC_MMC
```
Symbol: INGENIC_MMC [=y] 

Type : tristate 

Prompt: Ingenic(XBurst) MMC/SD Card Controller(MSC) support
...
#### 使用Emmc/SD驱动，配置INGENIC_MMC_MMC0
```
Symbol: INGENIC_MMC_MMC0[=y] 

Type : boolean 

Prompt: INGENIC_MMC MMC0 
 ...
 -> Ingenic(XBurst) MMC/SD Card Controller(MSC) support (INGENIC_MMC [=y]) 
```
 

#### 使用sdio驱动，配置INGENIC_MMC_MMC1
```
Symbol: INGENIC_MMC_MMC1[=y] 

Type : boolean 

Prompt: INGENIC_MMC MMC1 
...
[ 1.125120] Register interface [wlan0] MAC: 70:4a:0e:69:5e:ba 
...
驱动加载成功：
```
[ 1.575803] mmc0: new SDHC card at address e624

[ 1.593001] mmcblk0: mmc0:e624 SU04G 3.69 GiB 
...
AirKissdemo是为Wi-Fi设备提供的局域网发现和局域网通讯的技术。
...
ssid: JZ_SW /*微信端配置的网络账号*/

password: jz_sw%#!135 /*微信端配置的网络密码*/
...
/usr/bin/wifi_ap_mode_start.sh

注意: 使用wifi_ap_mode_start.sh脚本前需要修改,如下图所示:

0![Screenshot from 2021-09-03 10-14-17](assets/eMMC_SD_SDIO_接口.0.png)
