...
void binder_buf_move_buffer(tBinderBuf *b_src_buf, tBinderBuf *consumed_buf) {
  ...
  memmove(b_src_buf->ptr, b_src_buf->ptr + consumed_buf->consumed,
          unprocess_size);
  b_src_buf->consumed = unprocess_size;
}

int binder_io_init(tBinderIo *bio, void *data, size_t data_size,
  ...
  if (x_likely(bio) && x_likely(data)) {
    ...
    if (offsets_byte_len > (data_size >> 1)) {
      printf("%s, Error: offset byte len is too big, please alloc more data "
             "for binder io\n",
             __func__);
      return ret;
    }
...
int binder_io_append_data(tBinderIo *bio, char *data,int len) {
  int ret = -1;
  uint32_t *ptr;

  if (x_likely(bio)) {
    if (data) {
      ptr = (uint32_t *)binder_io_alloc(bio, sizeof(uint32_t) + len);
      if (ptr) {
        *ptr = len;
        memcpy((uint8_t *)(++ptr), data, len);
        ret = 0;
      } else {
...
static struct flat_binder_object *binder_io_obj_get(tBinderIo *bio,
                                                    uint32_t offset_idx) {
  struct flat_binder_object *flat_obj = NULL;
  size_t cur_off = bio->data - bio->data0;
  if (offset_idx < bio->offs_avail && cur_off == bio->offs[offset_idx]) {
    flat_obj =
        (struct flat_binder_object *)binder_io_get(bio, sizeof(*flat_obj));
  }
 ...
 */
void binder_io_init_from_txn(tBinderIo *bio,
...
int binder_io_get_data(tBinderIo *bio, uint8_t **data,size_t *sz) {
  /*first get the len of string*/
  uint32_t s_len = binder_io_get_uint32(bio);
  if (s_len == INVALID_STRING_TAG)
    return -1;
  ...
  *data = (uint8_t*)binder_io_get(bio, s_len);
