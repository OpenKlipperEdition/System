...
static void usage()
{
        ...
        printf("\neg: h264dec-example -pixel_size 1280x720 -i test.h264 -d /dev/fb0 -n 3\n\n");
        ...
        print_opt_help(4, "format of framebuffer, [nv12] or [bgr]\n");
        print_opt_help(5, "show help.\n");
}
...
unsigned char tmp_buf[640 * 480 * 4];
...
int main(int argc, char **argv)
{
        ...
        unsigned char *tmp_bgr_buffer = NULL;
        int x, y;
        ...
        if (!strcmp(input_params.fb_fmt, "bgr")) {
                ...
                tmp_bgr_buffer = malloc(h264dec_param.codecparam.h264d_param.src_width * h264dec_param.codecparam.h264d_param.src_height * 4);
        } else if (!strcmp(input_params.fb_fmt, "nv12")) {
                fb_attr.frame_fmt    = IMPP_PIX_FMT_NV12;
        } else {
                printf("Format of framebuffer parameter error.\r\n");
        ...
        IHal_SampleFB_SetTargetFrameSize(fb0_handle, input_params.width, input_params.height); //input_params.width / 2, input_params.height / 2); // TODO: 设置会死机.
        IHal_SampleFB_SetTargetPos(fb0_handle, x, y);
        ...
        dstbuf_nums = IHal_Codec_CreateDstBuffer(codec_handle, IMPP_EXT_USERBUFFER, input_params.nbuffers);
        assert(1 <= dstbuf_nums);


        int i;

        for (i = 0; i < input_params.nbuffers; i++) { // 3 是最大FrameBuffer数量，也是最大codec可用的数量.
                //下一帧用于显示的buffer，与VPU 建立联系，但是不进行实际的数据修改动作.
                IHal_SampleFB_GetMem(fb0_handle, &fb0_buf);
                IHal_Codec_SetDstBuffer(codec_handle, fb0_buf.index, &fb0_buf);
        }
        ...
        while (1) {
                ...
                do {
                        ...
                        fb0_buf.vaddr = codec_dststream.mp.vaddr[0]; //共用的buffer，与y地址相同.
#if 1
                        if (fb_attr.frame_fmt == IMPP_PIX_FMT_BGR_888) {
                                NV12_T_BGR((uint8_t *)codec_dststream.mp.vaddr[0], (uint8_t *)codec_dststream.mp.vaddr[1], tmp_bgr_buffer, h264dec_param.codecparam.h264d_param.src_width, h264dec_param.codecparam.h264d_param.src_height, 128);

                                memcpy((void *)fb0_buf.vaddr, (void *)tmp_bgr_buffer, h264dec_param.codecparam.h264d_param.src_width * h264dec_param.codecparam.h264d_param.src_height * 4);
                                //memset(fb0_buf.vaddr, 0, codec_dststream.mp.len[0]);
                        }
#endif
//			memset(codec_dststream.mp.vaddr[1], random()%255, codec_dststream.mp.len[1]);
                        /* 硬件绘图 */
#if 0
                        //动态修改显示位置.
                        //x = 1;
                        //y = 0;
                        IHal_SampleFB_SetTargetPos(fb0_handle, x, y);
                        IHal_SampleFB_SetTargetFrameSize(fb0_handle, x, y);
                        //IHal_SampleFB_SetAlpha(fb0_handle, x % 255);
                        IHal_SampleFB_SetAlpha(fb0_handle, 255);
                        //printf("---x: %d, y:%d\n", x, y);
                        IHal_SampleFB_CompRestart(fb0_handle);
#endif
                        ...
                        buffer_dqueued[codec_dststream.index] = 1;
                        int next_to_render = (codec_dststream.index + 1) % input_params.nbuffers;
                        if (buffer_dqueued[next_to_render] == 1) {
                                // GetNextBufferToRender, 1->2->0
                                // VPU index, 0, 1, 2.
                                //IHal_SampleFB_GetMem(fb0_handle, &fb0_buf);
                                IHAL_CodecStreamInfo_t next_codec_dststream;
                                IMPP_BufferInfo_t next_codec_buffer;

                                // 测试: 向Buffer填充数据，并延迟一段时间，如果DPU正在刷新当前Buffer，可以明显看到显示异常.
                                IHal_Codec_GetDstBuffer(codec_handle, next_to_render, &next_codec_buffer);
                                int s = random();
                                memset((void *)next_codec_buffer.mplane.vaddr[0], 0, next_codec_buffer.mplane.len[0]);
//				usleep(s % 30000);

                                next_codec_dststream.index = next_to_render;

                                // Rendering.
                                ret = IHal_Codec_QueueDstBuffer(codec_handle, &next_codec_dststream);
                                assert(0 == ret);
                                buffer_dqueued[next_to_render] = -1;
                        }
