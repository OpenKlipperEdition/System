...
TARGET_INSTALL_PATH:=$(OUT_DIR)/$(PRODUCT)/$(TARGET_PRODUCT)-$(TARGET_BUILD_VARIANT)
...
OUT_IMAGE_DIR:=$(TARGET_INSTALL_PATH)/image
...
SYSROOT:=prebuilts/toolchains/mips-gcc-glibc/$(DEVICE_COMPILER_PREFIX)/libc/$(arch_variant_libc_dir)
...
OUT_DEVICE_USER_DIR:=$(SYSROOT)/usr

OUT_HOST_DIR:=$(TARGET_INSTALL_PATH)/host
...
PKG_CONFIG_EXEC=$(TOP_DIR)/$(OUT_HOST_DIR)/bin/pkg-config
QMAKE_SPEC_FILE:=$(OUT_HOST_DIR)/usr/mkspecs/devices/linux-mips-manhaton-g++/qmake.conf
CMAKE_TOOL:=$(TOP_DIR)/$(OUT_HOST_DIR)/usr/bin/cmake
LIB_ENV:=$(TOP_DIR)/$(OUT_HOST_DIR)/usr/lib

BUILD_FREERTOS_INTERMEDIATE:=$(TOP_DIR)/$(OUT_DEVICE_OBJ_DIR)/freertos-intermediate
BUILD_FREERTOS_SYSTEM_MAP:=$(BUILD_FREERTOS_INTERMEDIATE)/System.map

RISCV_SDK_TOOLCHAIN_PATH:=prebuilts/mcu_sdk/riscv_toolchains
RISCV_SDK_SOURCECODE:=prebuilts/mcu_sdk/libbare-cpu

RISCV_CMAKE_TEMPLATE:=$(OUT_HOST_BINRARY_DIR)/cmake/riscv_toolchain.cmake

RTOS_MODULES:=
RTOS_SECTION_IMAGE:=
RTOS_MODULE_IMPORT_FUNC:=
