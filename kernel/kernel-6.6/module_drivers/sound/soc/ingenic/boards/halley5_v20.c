...
static const struct snd_soc_dapm_route audio_map[] = {
	...
	/* 2026-07-23 (beeper/BAIC4 audio investigation): removed the six routes
	 * that terminated at "DMIC capture"/"BAIC4 playback"/"BAIC4 capture" -
	 * those widgets were auto-generated by the now-removed "DMIC"/"BAIC4"
	 * DAI links (halley5_v20.c's x2000_dais[]), not declared statically, so
	 * routing to/from them after removing those links fails registration
	 * outright ("ASoC: Failed to add route ... snd_soc_register_card failed
	 * -19", confirmed live - a real regression from an earlier, incomplete
	 * pass at this same fix that only touched the DAI link array and missed
	 * these DAPM routes). LI0/LI6/LO4_MUX/LO5_MUX/LO9_MUX and the "DMIC"/
	 * "PCMIN"/"PCMOUT" widgets themselves are untouched - they're fixed
	 * hardware mixer ports (registered from as-dsp's own ingenic,li-port/
	 * lo-port properties) or statically-declared widgets, valid regardless
	 * of whether anything feeds them; an unconnected mux input/output is
	 * not a DAPM error the way a route to a nonexistent widget is.
	 */
};
