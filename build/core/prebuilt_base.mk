...
__local_path := ${strip $(LOCAL_PATH)}
__local_module_path := ${strip $(LOCAL_MODULE_PATH)}
include $(BUILD_SYSTEM)/choose_src_target_path.mk

$(call choose_src_target_path, $(LOCAL_COPY_FILES))

__out_module_file := $(__local_module_path)/$(LOCAL_TARGET_FILES)
__local_module_file :=$(__local_path)/$(LOCAL_SRC_FILES)

include $(BUILD_SYSTEM)/module_install.mk
include $(BUILD_SYSTEM)/mma_build.mk

ifeq ($(LOCAL_MODULE_CLASS),DIR)
__out_module := $(__local_stamp_build)@$(__out_module_file).dir@$(__local_module_file)@
