...
class Maker(object):
    ...
    def get_customer_effective_types(self):
        ...
        if config.Config.get_ota_mode() == "fullpkg":
    ...
    def start(self):
        ...
        if config.Config.get_singlepkg() == "no":
    ...
    def pack(self):
        ...
        outpath = os.path.realpath("%s/%s" %(config.Config.get_outputdir_path(),
                config.Config.get_customer_files_suffix()))
        os.chdir(outpath)
        if config.Config.get_singlepkg() == "yes":
            print("==========singlepkg==========")
            os.mkdir(config.output_package_name, 0o755);
            for i in range(0, packagecnt+1):
                package = ("%s%03d") % (config.output_package_name, i)
                shutil.move(package, "%s/" %(config.output_package_name))
            output_package_unencrypted = ("%s.unencrypted.zip" % (
                config.output_package_name))
            output_package_encrypted = ("%s.zip" % (config.output_package_name))
            caller_zip = "zip -ry %s %s" % (
                output_package_unencrypted, config.output_package_name)
            print("cmd:%s" %(caller_zip))
            ...
            if config.signature_flag:
                ...
                print("caller_signature:%s" %(caller_signature))
                os.system(caller_signature)

            sha1file = config.Config.get_outputdir_path() + "/sha1Tab"
            cmd="echo '"+self.calcSha1(os.path.abspath(output_package_encrypted))+"' >> "+ sha1file
            os.system(cmd)

            os.remove(output_package_unencrypted)
            shutil.rmtree(config.output_package_name)
        else:
            print("==========multipkg==========")
            for i in range(0, packagecnt+1):
    ...
    def cmdline_parse(self):
        ...
        help = '''ota mode diffpkg or fullpkg'''
        parser.add_argument('-m', '--otamode', help=help)
        help = '''The Image path is where you place the images'''
        parser.add_argument('-i', '--imgpath', help=help)
        help = '''The src path to diff'''
        parser.add_argument('-f', '--srcpath', help=help)
        help = '''The dst path to diff'''
        parser.add_argument('-t', '--dstpath', help=help)
        help = '''The Config path is where you place the .conf files'''
        parser.add_argument('-c', '--configpath', help=help)
        ...
        help = '''The limit size of difference package'''
        parser.add_argument('-l', '--limitsize', help=help, type=int)
        parser.add_argument('-s', '--slicesize', help=help, type=int)
        parser.add_argument('--singlepkg', help=help)
        ...
        if args.otamode:
            config.Config.set_ota_mode(args.otamode)
        if args.imgpath:
            config.Config.set_image_path(args.imgpath)
        if args.srcpath:
            config.Config.set_src_path(args.srcpath)
        if args.dstpath:
            config.Config.set_dst_path(args.dstpath)
        if args.configpath:
            config.Config.set_image_cfg_path(args.configpath);
        if args.output:
            config.Config.set_outputdir_path(args.output)
        if args.limitsize:
            config.Config.set_limitsize(args.limitsize*1024)
        if args.slicesize:
            config.Config.set_slicesize(args.slicesize)
        if args.singlepkg:
            config.Config.set_singlepkg(args.singlepkg)
        if config.signature_flag:
            if args.publickey:
