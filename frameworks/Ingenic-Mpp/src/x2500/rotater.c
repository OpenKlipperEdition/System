...
// if ok will return the index of buffer
...
IHal_Rot_Handle_t *IHal_Rot_CreateChan(IHal_Rot_ChanAttr_t *attr)
{
        ...
        if (attr->sWidth <= 0 || attr->sWidth > 3840 || attr->sHeight <= 0 || attr->sHeight > 2160) {
                IMPP_LOGE(TAG, "X2500 supports max image width of 3840 and max image height of 2160.");
                return IHAL_RNULL;
        }
