...
void construct_block_manager(struct block_manager* this, const char *blockname,
    ...
    if (retval < 0) {
        LOGE("Failed to init mmc block manager\n");
        goto out;
    }
#else
    ...
    if (bm == NULL) {
        LOGE("Block manager you request is not exist:%s\n",blockname);
        goto out;
    }
    ...
    if (get_system_platform() == XBURST2){
...
#ifdef MMC_DEVICE
    retval = mmc_manager_destroy();
    if (retval < 0) {
        LOGE("Failed to destory mmc block manager\n");
        goto out;
    }
#else
