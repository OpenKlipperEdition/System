...
struct encode *set_enc_config(struct camera_info *camera_inf)
{
    struct encode *encode = (struct encode *)malloc(sizeof(struct encode));
    encode->name = "helix";
    encode->width = camera_inf->param.width;
    encode->height = camera_inf->param.height;
    encode->pixfmt = camera_inf->fmt.fourcc;
    encode->fd = camera_inf->fd_helix;
    encode->buf_count = 1;
    encode->v4l2_buffers_count = 2;
    encode->buffers_count = 1;
    encode->planes_count = 2;
    encode->encoded_count = 0;

    return encode;

}

int jpeg_enc_init(struct camera_info *camera_inf)
{
    ...
    encode = set_enc_config(camera_inf);
    ...
    rc = helix_set_format(encode->fd, &setformat);
    if (rc < 0) {
    ...
    for (int i = 0; i < encode->buf_count; i++) {
	    ...
	    memset(buffers->source_data[j], 0, buffers->source_sizes[j]);

	    printf("buffer[%d]@%p, buffer->source_map[%d]: %x, buffer->source_sizes[%d]: %x\n", i, buffers, j, buffers->source_map[j], j, buffers->source_sizes[j]);
...
int video_engine_encode(int fd, __u32 index,
        ...
        struct encode *encode, uint8_t *p_dst)
{
	...
	sprintf(filename, "/mnt/jpeg-%d.jpg", encode->encoded_count++);

       	fd = open(filename, O_RDWR | O_CREAT, 0775);
	write(fd, buffers->destination_map[0], bytesused);
	close(fd);
	sync();
    }
#endif

    //save_write_jpeg_bs(p_dst, buffers, bytesused);
    save_memcpy_jpeg_bs(p_dst, buffers, bytesused);

    return bytesused;
}

int jpeg_enc_start(struct camera_info *camera_inf, uint8_t *p_dst)
{
    struct encode *encode = camera_inf->jz_jpeg;
    struct video_buffer *buffers = encode->buffers;

    encode->input_buf = camera_inf->buf_vaddr;
    __u32 v4l2_index = 0;
    __u32 i;

    __u32 bytesused = 0;

    static int xxx_index = 0;

#if 0

    printf("----------%s, %d, xxx_index: %d\n", __func__, __LINE__, xxx_index++);
    int fd = open("/mnt/camera_output-1.yuv", O_RDWR | O_CREAT | O_APPEND, 0755);
    if(fd < 0) {
    	perror("-----------------error open -------");
    }

    write(fd, encode->input_buf, buffers->source_sizes[0] + buffers->source_sizes[1]);

    close(fd);
#endif

    unsigned int offset = 0;
    memcpy(buffers->source_data[0],
		    encode->input_buf + offset,
		    buffers->source_sizes[0]);

    offset = encode->width * encode->height;
    memcpy(buffers->source_data[1],
		    encode->input_buf + offset,
		    buffers->source_sizes[1]);

    if(encode->planes_count == 3) {
	    offset = encode->width * encode->height  + encode->width * encode->height / 4;
	    memcpy(buffers->source_data[2],
			    encode->input_buf + offset,
			    buffers->source_sizes[2]);

    }

#if 0
    fd = open("/mnt/camera_output-2.yuv", O_RDWR | O_CREAT | O_APPEND, 0755);
    if(fd < 0) {
    	perror("-----------------error open -------");
    }

    write(fd, buffers->source_data[0], buffers->source_sizes[0]);
    write(fd, buffers->source_data[1], buffers->source_sizes[1]);
...
int jpeg_enc_stop(struct camera_info *camera_inf)
{
    struct encode *encode = camera_inf->jz_jpeg;
    struct video_buffer *buffers = encode->buffers;
    int rc = 0;
