![](assets/X2500-ISP-TUNING_API_应用开发指南.0.png)
**君正®**
...
int isp_tuning_set_wdr(int fd, int value);
...
int isp_tuning_set_default_bin_path(int fd, unsigned char *path);
...
| ---- | ------------------------------------------------------ |
| fd   | video节点的文件描述符                                  |
| path | bin路径，绝对路径 (例如:/etc/sensor/sc230ai-x2500.bin) |
...
int isp_tuning_switch_bin_path(int fd, unsigned char *path);

【功能描述】

切换Camera加载bin文件的路径（64byte）。
...
| ---- | --------------------------------------------------- |
| fd   | video节点的文件描述符                               |
| path | bin路径，绝对路径 (例如:/etc/sensor/sc230ai-ir.bin) |
...
int isp_tuning_set_hue(int fd, int value);
...
| 20    | Reserved    | Modification not recommended |
...
| 3     | Reserved    | Modification not recommended |
| 2     | AWB0        | 0: enable 1: disable         |
| 1     | LSC         | 0: enable 1: disable         |
| 0     | BLC         | 0: enable 1: disable         |
...
自定义Camera曝光。如果value = 0，自动曝光模式；如果value > 0，手动曝光模式，value即自定义的曝光值，范围1-Camera最大曝光值。（Camera最大曝光请查阅sensor手册）
...
| top    | 感兴趣区域的上边界（大于0）        |
| bottom | 感兴趣区域的下边界（小于图像高度） |
| left   | 感兴趣区域的左边界（大于0）        |
| right  | 感兴趣区域的右边界（小于图像宽度） |
...
该函数必须在Camera开流之后进行调用，且该函数需要循环调用，实时传入感兴趣区域坐标。

## 设置去雾强度

【函数原型】

int isp_tuning_set_defog(int fd, int enable, int dark, int middark, int midbright, 

int morebright, int bright);
...
int isp_tuning_set_uv_thr(int fd, int min, int max);
...
| ---- | ----------------------- |
| fd   | video节点的文件描述符   |
| min  | uv最小值 range[0-256]   |
| max  | uv最大值 range[min-256] |
...
自定义全局强光抑制。如果value = 0，非使能强光抑制；如果value > 0，使能强光抑制且设置等级，范围1-16。
...
| ----- | ------------------------------- |
| fd    | video节点的文件描述符           |
| value | disable : 0enable : range[1-16] |
...
获取全局强光抑制状态。如果value = 0，未使能强光抑制；如果value > 0，使能强光抑制且value为当前强光抑制等级，范围1-16。
...
自定义Camera 白平衡模式。共有10种模式，选择模式0和模式10时必须设置红色通道和蓝色通道数值，范围0 - 65535。
...
int isp_tuning_get_af_metric(int fd);
