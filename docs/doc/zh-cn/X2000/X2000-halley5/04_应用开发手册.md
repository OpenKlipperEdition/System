![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.0.png)
...
Halley5软件平台是君正开发的一套linux系统发布、开发的平台。平台通过Build.mk文件进行指导性编译。支持如下功能。
...
![sdk](assets/03-X2000-Halley5_平台应用开发手册-v2.0.1.png)

图 1-1 halley5平台目录结构

## 特别注意

该文档所有内容均以X2000平台下的halley5开发板为例进行说明，开发人员可根据自己实际下载到的代码进行参考。
...
halley5软件平台针对使用不同的存储介质，提供了几个默认配置，默认配置存放在工程`buildroot/configs/`文件夹下。如下为对应板级为halley5的几个不同存储介质的默认配置。
```
x2000_halley5_linux_defconfig

x2000_halley5_linux_lvgl_defconfig

x2000_halley5_msc_burn_defconfig 

x2000_halley5_msc_ota_defconfig 

x2000_halley5_nand_awtk_defconfig 

x2000_halley5_nand_qt_defconfig 

x2000_halley5_nor_defconfig 
```
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.2.png)
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.3.png)

图 2-1 君正厂商配置

### system config for rootfs的配置

在halley5平台中，可以对system config for rootfs进行一些系统的配置。主要包括如下几项配置
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.4.png)

图 2-2 君正厂商配置中rootfs可选配置内容
...
halley5平台中，可以对WIFI进行配置，以下为WIFI可配置的几个内容。
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.5.png)

图 2-3 君正厂商配置中wifi可选配置项
...
halley5平台中，可以配置pretest 对板载的模块进行测试，可以进行测试的功能如下所示。
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.6.png)

图 2-4君正厂商配置中pretest的可选择配置项
...
halley5平台中，可以配置蓝牙是否支持BSA server，以及选择使用的蓝牙芯片。具体的配置方法如下。

1. 进入halley5工程
2. 执行make buildroot-menuconfig
3. 配置Ingenic packages/bluetooth，配置选项的路径如下图所示。

![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.7.png)

图 2-5 君正厂商配置中bluetooth的配置路径

### h264e netlink server support配置

halley5平台中，可以配置是否支持h264e netlink server，具体的配置方法如下。

1. 进入halley5工程
2. 执行make buildroot-menuconfig
3. 配置Ingenic packages/h264e netlink server support，配置选项的路径如下图所示。

![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.8.png)

图 2-6 君正厂商配置中h264e netlink server support的配置路径 

### applypatch配置

halley5平台中，可以配置是否使用applypatch，具体的配置方法如下。

1. 进入halley5工程
2. 执行make buildroot-menuconfig
3. 配置Ingenic packages/applypatch，配置选项的路径如下图所示。

![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.9.png)
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.10.png)

配置的选择请参考《Halley5软件平台快速开发指南》

如下为平台编译的一些常用命令。进入halley5软件平台根目录，执行下列指令编译相应的内容。
...
单独编译buildroot后，生成的rootfs会存放在工程`obj/buildroot-intermediate/images/`目录下，当执行 ”make“ 指令整体编译后，会将生成的rootfs搬运到inage目录下。
...
$ make $(LOCAL_MODULE) 
```
即make “模块名” 就可单独编译单个模块，以工程`packages/example/App/cimutils`下的cimutils测试模块为例，此模块LOCAL_MODULE（Build.mk中）赋值为：cimutils，执行如下命令即可单独编译cimutils模块：
```bash
$ make cimutils
```
...
在halley5平台的工程`buildroot/packages/`目录下，有一些可供选择的软件包。当需要使用这些软件包时，可以通过如下方式进行编译。
```bash
$ make buildroot-<packname> 
```
如要编译工程`buildroot/packages/`下的ffmpeg，执行如下指令编译。
```bash
...
1. 模块内容改动后，需删除out目录下的中间文件，再进行编译。Cimutils产生的中间文件路径为: `out/product/halley5/obj/DEPANNER/cimutils-intermediate`,删除以cimutils为前缀的目录
2. 执行单独编译模块的指令编译模块。即：make cimutils
3. 执行make 指令，将生成的内容更新到工程`out/product/halley5/image/`下的系统镜像中。

注：packages下软件包生成的中间文件路径一般在工程`out/product/halley5/obj/DEPANNER/`或者工程`out/product/halley5/obj/out/product/halley5/obj/packages/example/App/`目录下。buildroot/下的软件包生成路径一般在工程`out/product/halley5/obj/buildroot-intermediate/build/`目录下。
...
v4l2-jpegenc用于VPU JPEG的编码，用于将nv12、nv21和tile420格式文件转换为JPEG格式文件。

* 源码路径：packages/example/App/v4l2-jpegenc/src
* 软件流程图如下：

![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.11.png)

图 3-1 君正平台应用V4l2-jpegenc软件流程图

### 使用方法

#### 模块编译

进入halley5平台的根目录，执行如下指令进行模块编译。
```bash
$ make v4l2-jpegenc
...
模块编译完成后，生成可执行文件v4l2-jpegenc，存放在工程`out/product/halley5/system/usr/bin/`目录下。

#### 模块使用

在halley5开发板上运行如下指令运行模块。
```bash
# v4l2_jpegenc -v /dev/video1 -f filename.yuv -w 1280 -h 720 -o /tmp/output.jpg
```
...
查看编码后的output.jpg文件是否显示正常。

##  v4l2-jpegdec

### 功能简介

v4l2-jpegdec用于VPU JPEG的解码，用于将JPEG格式文件转换为nv12格式文件。

* 源码路径：packages/example/App/v4l2-jpegdec/src
* 软件流程图如下：

![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.12.png)

图 3-2君正平台应用V4l2-jpegdec软件流程图
...
$ make
```
 模块编译完成后，生成可执行文件v4l2-jpegdec，存放在工程`out/product/halley5/system/usr/bin/`目录下。

#### 模块使用

在halley5开发板上执行如下指令运行模块。
```bash
# v4l2_jpegdec -v /dev/video1 -f filename.jpg -w 1280 -h 720 -o /tmp/output.yuv
```
...
查看解码后的output.yuv文件是否显示正常。

##  v4l2-h264enc

### 功能简介

V4l2-h264enc用于VPU H264的编码，用于将nv12、nv21和tile420格式文件转换为h264格式文件。

* 源码路径：packages/example/App/v4l2-h264enc/src
* 软件流程图如下：

![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.13.png)

图 3-3君正平台应用v4l2-h264enc软件流程图
...
模块编译完成后,生成可执行文件v4l2-h264enc，存放在工程`out/product/halley5/system/usr/bin/`目录下。

#### 模块使用

在halley5开发板上执行如下指令运行模块。
```bash
# v4l2_h264enc -t nv12 -v /dev/video1 -f filename.yuv -w 1280 -h 720 -o /tmp/output.h264
```
...
查看编码后的output.h264文件是否显示正常

##  v4l2-h264dec

### 功能简介

v4l2-h264dec用于VPU h264的解码，用于将h264格式文件转换为nv12、nv21格式文件。

源码路径：packages/example/App/v4l2-h264enc/src

软件流程图如下：

![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.14.png)

图 3-4君正平台应用V4l2-h264dec软件流程图
...
模块编译完成后,生成可执行文件v4l2-h264dec，存放在工程`out/product/halley5/system/usr/bin/`目录下。
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.15.png)
...
模块编译完成后,生成可执行文件cimutils，存放在工程`out/product/halley5/system/usr/bin/`目录下。
...
webcam_gadget测试UVC输出

* 源码路径：packages/example/App/usb-gadget/webcam
* 软件流程图如下：

![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.16.png)
...
模块编译完成后,生成webcam_gadget，存放在工程`out/product/halley5/system/usr/sbin/`目录下。
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.17.png)
...
模块编译完成后,生成v4l2-isp-tuning，存放在工程`out/product/halley5/system/usr/bin/`目录下。
...
进入halley5平台的根目录，执行如下指令进行编译。
```bash
$ make mjpg-streamer

$ make
```
 模块编译完成后,生成mjpg-streamer，存放在工程`out/product/halley5/system/usr/bin/`目录下。同时，会生成一些库文件，存放在工程`out/product/halley5/system/usr/lib/mjpg-streamer`目录下。
...
1. 在halley5开发板上执行如下指令开启摄像头的图像采集
```bash
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.18.png)

##  stereo

### 功能介绍

stereo是mjpg-streamer中用于标定的一个库

* 源码路径：packages/example/App/stereo_demo

### 使用方法

在mjpg-streamer标定功能中被调用

####  模块编译

当编译mjpg-streamer时，会先编译该模块，一般不用单独编译该模块，单独编译该模块的方法为：进入halley5平台的根目录，执行如下指令进行编译。
```bash
$ make stereo-demo
```
...
device_base.mk用于配置工程编译的模块。位于`device/x2000halley5/`板级目录下。其中的宏PRODUCT_MODULES用于添加或删减通用工程编译的模块。在该文件中还有针对不同文件系统和存储介质的默认配置。这些都可以根据自己的需求进行个性化配置。

##  rootfs-overlay文件夹介绍

rootfs-overlay文件夹在`device/x2000halley5/`板级目录下，该文件夹下的文件在编译生成文件系统时，会替换文件系统中的同名文件。从而可以个性化的配置自己的文件系统。
...
在halley5平台中可以直接配置安装adb工具，方法如下：

1. 进入halley5工程
2. 执行make buildroot-menuconfig
3. 配置Ingenic packages/system config for rootfs/enable adb service,如下图所示。

![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.19.png)
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.20.png)
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.21.png)
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.22.png)
...
![lrzsz](assets/03-X2000-Halley5_平台应用开发手册-v2.0.23.png)
...
![sz](assets/03-X2000-Halley5_平台应用开发手册-v2.0.24.png)
...
![1](assets/03-X2000-Halley5_平台应用开发手册-v2.0.25.png)

图 5-8 lrzsz工具的rz命令

* 按ctrl+A+Z，再按下S（Send file）

![2](assets/03-X2000-Halley5_平台应用开发手册-v2.0.26.png)

图 5-9使用lrzsz工具时minicom的配置截图

* 选择zmodem,进入文件选择界面

![3](assets/03-X2000-Halley5_平台应用开发手册-v2.0.27.png)

图 5-10使用lrzsz工具时文件选择页面截图 

* 上下键或者PageUp/PageDown移动，空格选择文件

![4](assets/03-X2000-Halley5_平台应用开发手册-v2.0.28.png)
...
1. 可执行bin文件
2. 本示例为工程`packages/example/App/cimutils/Build.mk`
```c
LOCAL_PATH := $(my-dir)
...
1. 生成动态库
2. 本示例为工程`external/tinyalsa/Build.mk`
```c
LOCAL_PATH := $(my-dir)
...
1. 本示例为工程`packages/example/App/linphone-desktop/Build.mk`
```c
LOCAL_PATH := $(my-dir)
...
halley5平台提供一个rootfs-overlay机制用于修改生成文件系统中的内容。rootfs-overlay是一个文件夹，该文件夹位于`device/x2000halley5/`板级目录下。当该文件夹下存在与文件系统中相同文件名的文件时，会用当前目录下的文件替换文件系统中的文件。

## 制作根文件系统

halley5平台支持多种类型文件系统的制作，可以通过配置文件系统的参数，制作不同的文件系统，下面是几个具体的文件系统制作方法。
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.29.png)

图 6-1 jffs2文件系统的参考配置

配置完成后，在工程目录中执行make指令进行编译，编译生成的文件会存放在工程`out/product/halley5/image/`目录下。
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.30.png)
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.31.png)
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.32.png)
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.33.png)
...
gdbserver可以使用源码编译产生，halley5中提供了gdb的源码。在buildroot/dl/gdb/目录下。

halley5中也提供了编译完成了gdbserver，用户只需将该gdbserver文件拷贝到板子中即可，其路径如下：
```
...
![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.34.png)

图 7-3 目标板执行指令后等待连接

![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.35.png)

图 7-4 宿主机进入gdb调试界面并连接目标板

![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.36.png)

图 7-5 目标板与宿主机成功建立连接

![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.37.png)

图 7-6 宿主机进行调试

![](assets/03-X2000-Halley5_平台应用开发手册-v2.0.38.png)
...
在halley5工程顶层目录下，可通过以下命令进行工程交叉调试配置
...
![Screenshot from 2020-11-27 11-37-15](assets/03-X2000-Halley5_平台应用开发手册-v2.0.39.png)

图 7-8 vscode界面截图

在halley5软件工程顶层目录下，可通过以下命令进行工程配置信息清除
```bash
