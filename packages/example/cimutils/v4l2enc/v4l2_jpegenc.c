...
int save_write_jpeg_bs(FILE *file, struct video_buffer *buffer, __u32 bytesused)
{
    int ret = 0;

    ret = fwrite(buffer->destination_map[0], sizeof(__u8),  bytesused, file);
    if (ret != bytesused) {
        ...
        struct encode *encode, FILE *fp)
{
    ...
    save_write_jpeg_bs(fp, buffers, bytesused);

    return bytesused;
}

int jpeg_enc_start(struct camera_info *camera_inf, FILE *fp)
{
    ...
    /*INPUT*/
    for(i = 0; i < encode->planes_count; i++) {
        memcpy(buffers->source_map[i],
                encode->input_buf + i * buffers->source_sizes[abs(i-1)],
                buffers->source_sizes[i]);
    }
    /*ENCODE*/
