...
该文档针对君正系列芯片，Kernelx.x 标准内核平台进行研发，主要针对内核的基本结构，内核的编译，设备树和驱动的配置和裁剪进行说明。 
...
![Screenshot from 2021-08-19 17-49-18](assets/内核开发简介.0.png)
...
out/product/x1600halley6.v10_nand_x.x.x-eng/obj/kernel-intermediate/arch/mips/boot/uImage， 该文件就是最终需要烧录的内核镜像。

基于君正SDK的开发流程，源码和编译生成的中间文件是分离开的，中间文件会生成在out/product/x1600halley6.v10_nand_x.x.x-eng/obj/kernel-intermediate/目录下，包括内核的.config文件。

**注意事项：**

1. 使用君正SDK的开发流程，需要保证源码目录没有被污染，即不能即在源码目录下使用传统的make xx_defconfig进行内核配置和编译，否则会报错误。此时可以在kernel源码目录下执行
```
$ make distclean
...
所有针对开发板的内核配置默认配置文件都存储在arch/mips/configs/目录下。
...
| --- | --- |
| **配置文件** | **说明** |
| **x1600_halley6_v10_linux_defconfig** | 用于主存储为nand/nor启动的配置 |

表 1 Halley6 kernel4.4.94 内核配置表
...
| --- | --- |
| **配置文件** | **说明** |
| **x1600_halley6_v10_linux_defconfig** | 用于主存储为nand/nor启动的配置,支持halley6_v10板级配置 |
| **x1600_halley6_v20_linux_defconfig** | 用于主存储为nand/nor启动的配置，支持halley6_v20板级配置 |

表 2 Halley6 kernel5.10 内核配置表
...
关于设备树的配置文件，主要存放路径为arch/mips/boot/dts/ingenic目录, dts的组成一般是soc级定义和board级的定义。

![Screenshot from 2021-08-17 10-26-11](assets/内核开发简介.2.png)

图表 1‑3 dts结构示意图

Halley6 平台涉及到的devicetree文件如下表:
...
| --- | --- |
| **dts文件** | **文件说明** |
| **halley6_v10.dts** | 针对Halley V1.x系列的开发板板级配置。自定义开发板配置时，需要修改此文件 |
| **Halley6_cameras****├── RD_X1600_HALLEY6_MIPI_CAMERA.dtsi****├── RD_X1600_HALLEY6_DVP_CAMERA.dtsi** | 此文件夹下的内容针对的是halley6支持的Camera子板配置，如果需要添加新的camera配置，可以参考里面的内容进行添加。如果完全使用官方的配置，不需要修改内容 |
| **x1600.dtsi** | X1600 SOC 芯片基础配置，客户一般不需要修改。 |
| **x1600-pinctrl.dtsi** | X1600 GPIO function配置，客户一般不需要修改。 |
...
1. 需要修改配置文件， 如： u-boot/include/configs/x1600_halley6.h,添加devicetree支持
```c
...
会在out/product/x1600halley6/obj/kernel-intermediate/下生成文件

***arch/mips/boot/dts/ingenic/halley6_v10.dtb***
...
2. **烧录eMMC/SD DTB 文件**
	...
	2. 如果需要自己指定dtb分区，需要做如下修改：
		1. 以halley6为例，需要参考文件u-boot/board/ingenic/”板级”/partitiongs.tab
...
例： arch/mips/boot/dts/ingenic/halley6_v10.dts
```c
/ {
 ...
 bootargs = "console=ttyS1,115200 mem=128M@0x0ip=off init=/linuxrc ubi.mtd=3 root=ubi0:rootfs ubi.mtd=4 rootfstype=ubifs
