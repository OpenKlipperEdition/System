...
1. 在module_drivers/drivers/video/fbdev/ingenic/fb_stage/displays/下新建相关屏的文件
...
***module_drivers/drivers/video/fbdev/ingenic/displays/Kconfig***
...
在**X2660_HALLEY_MIPI_LCD_FW050.dtsi**中配置如下：
...
| ingenic,vdd-en-gpio | 供电引脚：PC08 |
| ingenic,rst-gpio | 复位引脚：PC15 |
| ingenic,lcd-pwm-gpio | 背光引脚：PE02 |
...
用户可根据实际需求关闭dpu设备和rotate设备，将以上节点配置为disabled。
...
* 配置兼容的屏（compatible）
* 根据不同型号的屏选择控制引脚(pinctrl-0)，不同的引脚设置在**module_drivers/dts/x2600-pinctrl.dtsi**
...
&pwm {
    ...
    pinctrl-0=<&pwm10_pc>; /*按照实际情况配置pwm的gpio*/ 
...
backlight {
    ...
    pwms=<&pwm 10 1000000>; /*选择pwm1控制背光，设置period1000000ns*/
...
测试涉及rdma写回功能，需要在LCD设备树中开启rdma模式：

```c
ingenic,disable-rdma-fb = <0>;
```

以x2660 halley v1.0为例，在X2660_HALLEY_MIPI_LCD_FW050.dtsi中修改dpu配置如下：
...
dpu功能测试使用IMPP测试demo：dpu-osd-example

测试所需图像文件：720x1280.argb 640x480.nv12，测试图像文件需要与测试demo在同级目录下
...
```
echo 6 > /sys/devices/platform/ahb0/13050000.dpu/layer0/src_fmt /*指定LCD显示格式*/
