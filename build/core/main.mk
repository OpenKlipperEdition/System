...
subdir_makefiles := \
	...
	$(shell $(BUILD_SYSTEM)/tools/findleaves.py --prune=$(OUT_DIR) --prune=.repo --prune=.git --prune=kernel --prune=u-boot --prune=prebuilts --prune=build --prune=buildroot --prune=device $(subdirs) Build.mk)
...
ALL_MODULES_CLEAN_TMP = $(filter-out $(ALL_MODULES_CLEAN_FILTER),$(ALL_MODULES_CLEAN))
ALL_MODULES_CLEAN := $(sort $(ALL_MODULES_CLEAN_TMP))
define CLEAN_MODULE
.PHONY: $(1)
$(1):|$(2)
...
include $(BUILD_SYSTEM)/build_rtos.mk
else
ifeq (x$(strip $(OTA)),x)
...
clean: $(THIRDPART_CLEAN_MODULES)
	@echo
	@echo -e "\e[0;31m************************ Info ***************************"
	@echo The whole dirs will be removed:
	@echo - $(TARGET_INSTALL_PATH)
	@echo -n -e "\e[00m"
	@echo "Warning: Entire build direcotry will be removed, continue? [Y/n]"
	@read line; if [ $$line != "Y" ]; then echo aborting; exit 1 ; fi
	@echo "Start removing build dirs ...."
	@rm -rf docs/html
	@rm -rf docs/latex
...
help:
	@echo "------------------------- Help Info -----------------------------------"
	@echo "Building:"
	@echo " make			- compile the whole system, including uboot/kernel/buildroot/packages, post-image will take a long time."
	@echo " make clean		- clean the whole system, after 'make clean', you need to run 'make' for the whole system building."
	@echo " make uboot		- compile uboot only."
	@echo " make uboot-clean	- clean uboot."
	@echo " make kernel		- config & build kernel. this will apply defconfig under device dirs for the first time."
	@echo " make kernel-menuconfig  - config kernel, should run 'make kernel' firstly."
	@echo " make kernel-clean	- clean kernel, including configs."
	@echo " make buildroot		- config & build buildroot. this will apply the defconfig and generate host utils."
	@echo " make buildroot-rebuild" - build buildroot after reconfigured by custom.
	@echo "Modules building:"
	@echo " make show-modules	- print supported modules."
	@echo " mm			- building module under dirs where there is 'Build.mk'"
	@echo " mm [module]-clean	- clean a module under dirs where there is 'Build.mk'"
	@echo " make [module]		- building module with [module_name], module_name can be list by 'make show-modules'"
	@echo " make [module]-clean	- clean a module."
	@echo "Post Image:"
	@echo " make systemimage	- build rootfs applications and post image.."
	@echo " make post-image		- generate target files system images."
	@echo "----------------------------------------------------------------------"
...
include $(BUILD_SYSTEM)/build_qmake_config.mk
include $(BUILD_SYSTEM)/riscv_env.mk
