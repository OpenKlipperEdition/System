...
module_drivers/dts/x1600-pinctrl.dtsi或module_drivers/dts/x1600/x1600-pinctrl.dtsi

在 module_drivers/dts/"板级".dts或module_drivers/dts/x1600/"板级".dts 会根据驱动配置， 选中相应的设备功能管脚。内核的 gpio 驱动基于 gpio 子系统实现，很方便的进行 gpio 控制。
...
***module_driver/dts/x1600/x1600-pinctrl.dtsi***
...
其中x1600不支持滤波、高阻态、驱动能力、施密特输入和转换速率的设置。
...
例如：
```c
console_uart0:console-uart0 {

ingenic,pinmux = <&gpb 7 10>;

ingenic,pinmux-funcsel = <PINCTL_FUNCTION0>;

ingenic,pincfg = <&gpb 7 10 INGENIC_GPIO_PULLUP>;
...
kernel4.4.94和kernel5.10的差异主要体现在16.4.1章节，kernel5.10不再提供通过menuconfig关闭gpio组中断的功能了。
...
由此可以确定gpioa这组引脚的基准值就是0，其他gpio组同样道理。
