...
// Extend xburst2 MXU3 registers, as PTRACE_GETFPXREGS;
// struct xburst_mxu_struct export from kernel.
...
void dump_registers_mxuv3(log_t *log, pid_t tid) {
  struct xburst_mxu_struct r;
  if (ptrace(PTRACE_GETFPXREGS, tid, 0, &r)) {
    _LOG(log, logtype::ERROR, "cannot get fpx(mxuv3) registers: %s\n", strerror(errno));
    return;
  }
  ...
  for (int c = 0; c < 32; c++) {
    ...
    int off = 0;
    for (int i = 0; i < 64; i++) {
      snprintf(rval+off, 3, "%02x", r.vpr[c].val512[63-i]);
      off += 2;
      if ((i+1)%4 == 0 && i != 63)
        snprintf(rval + (off++), 2, "%c", ' ');
    }
    _LOG(log, logtype::MEMORY, "vrd%02d %s\n", c, rval);
  }
  ...
  for (int c = 0; c < 4; c++) {
    ...
    int off = 0;
    for (int i = 0; i < 64; i++) {
      snprintf(rval+off, 3, "%02x", r.vsr[c].val512[63-i]);
      off += 2;
      if ((i+1)%4 == 0 && i != 63)
        snprintf(rval + (off++), 2, "%c", ' ');
    }
    _LOG(log, logtype::MEMORY, "vrs%02d %s\n", c, rval);
  }
  _LOG(log, logtype::REGISTERS, "mcsr %08x\n",R(r.csr));
}
