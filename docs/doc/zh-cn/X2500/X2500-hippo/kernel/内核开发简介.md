...
![内核结构](assets/内核开发简介.0.png)

图 1‑1 内核驱动适配结构

内核驱动目录说明：为了方便开发查找相关驱动源码及驱动配置，现将君正芯片平台的相关驱动放在/module_drivers，其目录结构和内核原有结构保持不变。

内核驱动配置说明：在执行内核配置命令后，在配置界面，按照图1-2进入驱动配置界面
...
![X2000 内核结构1](assets/内核开发简介.3.png)
...
| --- | --- |
| **配置文件** | **说明** |
| **hippo_v10_linux_defconfig** | 用于hippo_v10板级内核默认配置 |
| **hippo_v10_linux_sfc_nand_recovery_defconfig** | 用于hippo_v12板级OTA 升级的recovery配置 |
| **hippo_v12_linux_defconfig** | 用于hippo_v12板级内核默认配置 |

表 1 Hippo kernel4.4.94 内核配置表
...
| --- | --- |
| **配置文件** | **说明** |
| **hippo_v10_linux_defconfig** | 用于hippo_v10板级配置 |
| **hippo_v12_linux_defconfig** | 用于hippo_v12板级配置 |

表 2 Hippo kernel5.10 内核配置表

## 设备树配置

devicetree用于描述芯片和开发板板级资源，主要有两种使用方式，一种方式直接和内核编译在一起，另外一种方式单独编译，单独存储，由uboot传参告知dtb存储的位置。

默认直接编译内核的方式，具体区别可以参考下文。

### 设备树文件介绍

关于设备树的配置文件，主要存放路径为module_drivers/dts/目录, dts的组成一般是soc级定义和board级的定义。

Hippo 平台的涉及到的devicetree文件如下表:
...
| --- | --- |
| **dts文件** | **文件说明** |
| **hippo_v10.dts** | 针对hippo V1.0系列的开发板板级配置；自定义开发板配置时，可根据需求进行修改。 |
| **hippo_v12.dts** | 针对hippo V1.2系列的开发板板级配置；自定义开发板配置时，可根据需求进行修改。 |
| **hippo_cameras** | 此文件夹下的内容针对的是Hippo支持的Camera子板配置，如果需要添加新的camera配置，可以参考里面的内容进行添加。 |
| **x2500-pinctrl.dtsi****x2500.dtsi** | X2500硬件资源配置，客户一般不需要修改。 |

表 3 Hippo kernel 相关dts文件
...
1. 需要修改配置⽂件， 如： u-boot/include/configs/hippo.h,添加devicetree支持
...
![/home/user/Pictures/dtbik.pngdtbik](assets/内核开发简介.4.png)

图表 1‑2 去掉INGENIC_BUILTIN_DTB
...
会在out/product/hippo.v12_nand_x.x.x-eng/obj/kernel-intermediate/下生成文件

arch/mips/boot/dts/ingenic/hippo_v12.dtb
...
![/home/user/Pictures/烧录.png烧录](assets/内核开发简介.5.png)
...
2. **烧录eMMC/SD DTB 文件**
	...
	2. 如果需要自己指定dtb分区，需要做如下修改：
		1. 以hippo为例，需要参考文件u-boot/board/ingenic/”板级”/partitiongs.tab
...
![/home/user/Pictures/dtb.pngdtb](assets/内核开发简介.6.png)
...
例： arch/mips/boot/dts/ingenic/hippo_v12.dts
...
/ {
 ...
 bootargs = "console=ttyS1,115200 mem=256M@0x0ip=off init=/linuxrc ubi.mtd=3 root=ubi0:rootfs ubi.mtd=4 rootfstype=ubifs rw";
